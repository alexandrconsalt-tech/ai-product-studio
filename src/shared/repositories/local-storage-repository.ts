...
function withTranscriptionSummaryModule(snapshot: RepositorySnapshot): RepositorySnapshot {
  ...
  const summaryV2Project = {
    ...
    name: "Конвейер саммари v2",
    description: "Независимый 13-этапный конвейер: проверенное хранилище, пять параллельных проверок, контроль качества v2 и безопасная публикация в CRM.",
    status: "testing" as const,
  ...
  const summaryV2Product = {
    ...
    status: "ready" as const,
    idea: {
      statement: "Формировать доказательное AI-саммари звонка с независимой оценкой качества и публикацией только после прохождения контроля качества.",
      source: "Требования к конвейеру саммари v2",
    },
    ...
    users: [
      ...
      { id: "user_summary_v2_quality", name: "Специалист качества", segment: "Качество AI" },
    ],
    jtbd: [{
      statement: "После звонка получить краткое проверенное саммари, чтобы продолжить работу без повторного прослушивания и без риска записать выдуманные данные в CRM.",
      context: "После клиентского звонка",
      desiredOutcome: "Проверенное саммари и отдельные атрибуты CRM опубликованы только при достаточном качестве",
    }],
    features: [
      { id: "feature_summary_v2_verified_store", name: "Хранилище разговора v2", description: "Сохраняет только данные с подтверждающими цитатами.", priority: "high" as const },
      { id: "feature_summary_v2_judges", name: "Пять независимых проверок", description: "Параллельно оценивают достоверность, полноту, полезность, договорённости и формат.", priority: "high" as const },
      { id: "feature_summary_v2_gate", name: "Контроль качества v2", description: "Разделяет технические ошибки, необходимость ручной проверки и решения о публикации.", priority: "high" as const },
    ],
    mvp: "Полный проход транскрипции через 13 этапов, подробный технический отчёт, отдельная история запусков и публикация в CRM v2.",
    metrics: [
      { name: "Итоговая оценка качества", target: ">= 95", category: "quality" as const },
      { name: "Доля технически успешных запусков", target: "100% без необработанных исключений", category: "success" as const },
      { name: "Критические галлюцинации", target: "0", category: "quality" as const },
    ],
    prd: "Конвейер саммари v2: проверка транскрипции, единое извлечение данных, нормализация, проверка доказательств, хранилище v2, генерация, пять параллельных проверок, контроль качества и публикация в CRM.",
    frameworkIds: ["framework_evaluation"],
    valueProposition: "Воспроизводимое доказательное саммари без каскадных исправляющих агентов и ложных успешных оценок.",
    targetAudience: "Команды продаж и контроля качества AI, тестирующие саммари звонков.",
    acceptanceCriteria: "Версия 2 запускается независимо от версии 1, сохраняет полный отчёт, не публикует результаты при технической ошибке или необходимости ручной проверки и не присваивает техническим ошибкам оценку.",
    createdAt: SUMMARY_V2_CREATED_AT,
    updatedAt: SUMMARY_V2_CREATED_AT,
    version: "2.0.0",
  };
