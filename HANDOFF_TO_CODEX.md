...
9. **`docs/NEW_SUMMARY_PIPELINE_SPEC.md`** — read this first (before writing any code) if your task touches the second product, «Анализ звонков v2» (`src/features/call-summary-pipeline/`). It is that product's own authoritative spec/PRD, added 2026-07-26 (see `CLAUDE.md`'s matching addendum). Never use it to justify changing the original call-transcription pipeline (`public/pipeline-lab-v3.html`).
