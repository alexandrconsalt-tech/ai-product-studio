...
**12.3 Edges.** `Edge { id, sourceNodeId, targetNodeId, sourcePortId?, targetPortId?, condition?: { field, operator, value, description? }, version }` — no lifecycle, embedded value object. `condition` was a free-text `expression: string` until 2026-07-03; it is now structured (`operator` one of `eq|neq|gt|gte|lt|lte`), matching the only real use case in the repository (the demo pipeline's confidence-based validation → store/human-review split, e.g. `{field: "confidence", operator: "gte", value: 0.72}`). There is still **no evaluator anywhere in the codebase** that actually reads and executes a condition at runtime — the struct is parseable and type-safe, but nothing calls it yet. Closing that remains part of the Pipeline DSL gap — see §14.

**12.4 The Simulation Engine — what it actually does (`src/shared/simulation/simulation-engine.ts`), stated precisely so no one mistakes it for a real runtime. Updated 2026-07-03 (§63 debt item 11, resolved) — it now reads some `Node` config, still fully synthetic:**
...
- Assuming `Edge.condition` is evaluated at runtime because it's now a structured, type-safe shape — the schema exists (§12.3), but no evaluator reads it yet; define one (§14) if you need to actually execute it.
...
**14.1 Current state.** "Pipeline DSL" today is: the `Node`/`Edge` Zod schemas (structural DSL for the graph shape) plus free-text `metadata: Record<string,string>` per node (e.g. the demo pipeline uses `metadata.threshold = "0.72"`, `metadata.sla = "15m"`, `metadata.retention = "90d"`, `metadata.format = "json"` — all untyped strings, not a schema, still open debt) plus, as of 2026-07-03, a **structured** `Edge.condition: { field, operator, value, description? }` (§12.3, §63 debt item 6 resolved) with **no evaluator anywhere in the codebase** — the shape is now type-safe and parseable, but nothing executes it at runtime yet.
...
6. **~~`Edge.condition.expression` and `Node.metadata` are untyped strings~~ — PARTIALLY RESOLVED 2026-07-03**: `Edge.condition` is now a structured, typed `{field, operator, value}` shape (§12.3, §14.2). `Node.metadata` remains an untyped `Record<string,string>` — still open, no evaluator exists for either yet regardless of typing (§14.1).
...
11. **~~Simulation Engine ignores `Node.promptId`/`modelId`/`temperature` entirely~~ — PARTIALLY RESOLVED 2026-07-03** (§12.4): the pipeline's recognized summary node (`promptId: "prompt_call_summary"`) now drives structured output shape, confidence (from `temperature`), and cost/latency (from `modelId`). Still open: any *other* node's `promptId`/`modelId`/`temperature` (e.g., the demo pipeline's validation node) remains unused, and there is still no real model call anywhere — this is a better simulation, not a real runtime.
12. **Pipeline canvas renders 0 edges** — found 2026-07-03, confirmed present in the pre-roadmap baseline (not a Roadmap regression). Root-caused by DOM/computed-style inspection (2026-07-03), attempted fix deliberately **not applied** because it could not be verified to fully resolve the issue (see below) — do not "fix" this with a quick CSS guess without re-verifying both findings in-browser. Two distinct, stacked causes found:
   - **Cause A (narrow viewport, reproducible, well-understood):** at viewport widths below ~1280+360px (Sidebar 248px + a positive canvas width + NodeInspector's fixed `w-[360px]`), the row `<div className="flex min-h-0 flex-1">` in `pipeline-screen.tsx` has to shrink its two children (`<div className="min-w-0 flex-1">` wrapping `<ReactFlow>`, and the `w-[360px]` `NodeInspector` aside). Because the canvas wrapper's Tailwind `flex-1` resolves to `flex: 1 1 0%` (flex-basis **0**), CSS flexbox's shrink algorithm — which activates because the two children's combined basis (0 + 360) exceeds the available row width — distributes 100% of the required shrinkage to whichever sibling has nonzero basis (the aside), leaving the canvas wrapper locked at exactly **0px width** (confirmed via `getBoundingClientRect()`/`getComputedStyle()`: canvas width 0, aside width = full row width). This is standard, correct CSS behavior for that combination of flex properties — it is a real bug in this component's layout, not a browser quirk.
   - **Cause B (persists even above 1280px, not yet root-caused):** at the documented minimum supported width (1280px, `docs/ux/GLOBAL_LAYOUT.md`), the canvas wrapper DOES get a positive width (measured 312px), yet the console still repeats `[React Flow]: The parent container needs a width and a height to render the graph` continuously, `.react-flow__edge` count is still 0, and `fitView` visibly frames the wrong region (2 of 8 nodes visible, rest off-screen) — consistent with React Flow computing its initial `fitView` transform from a container size read *before* the flex layout settles (the component's own `useEffect`-driven `flowNodes`/`flowEdges` state means nodes/edges are set on a render pass after mount), and never re-running `fitView` once the real size is available. This second cause was **not** fixed or confirmed fixable in this pass — it needs a focused investigation (likely: force a `ReactFlow` remount or call its imperative `fitView()` API after the container's `ResizeObserver` reports a stable non-zero size, rather than relying on the static `fitView` boolean prop) with the fix verified in-browser (edge count > 0, all 8 nodes visible) before being marked resolved.
