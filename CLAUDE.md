...
- `LocalStorageProjectRepository` (`src/shared/repositories/local-storage-repository.ts`) exposes `upsertX`/`deleteProject`-style methods, but the actual Zustand stores (e.g. `project-store.ts`) largely manipulate the `RepositorySnapshot` directly via `setSnapshot` rather than calling those repository methods — meaning the repository's mutation API is a **partially parallel, partially unused surface**. Before extending it, check whether the store already has its own path for the same mutation.
...
├── shared/       Cross-cutting: ui/, stores/, repositories/, simulation/, config/, lib/, hooks/(empty), api/(empty), model/(empty)
...
- `Edge, Framework, KnowledgeModule, Model, Node, Prompt` — **no lifecycle field at all**, by deliberate documented choice. Do not add one speculatively.
...
**Purpose.** Describe how AI pipelines are modeled and executed (simulated) today, and what changes when a real runtime replaces the simulation.
...
**12.3 Edges.** `Edge { id, sourceNodeId, targetNodeId, sourcePortId?, targetPortId?, condition?: { expression, description? }, version }` — no lifecycle, embedded value object. Conditional routing (used in the demo pipeline for the confidence-based validation → store/human-review split) is expressed via the `condition.expression` string field — there is **no formal expression language defined anywhere in the repository**; today it's just descriptive metadata, not parsed/evaluated by any code. This is the biggest concrete gap between "looks like a pipeline DSL" and "is one" — see §14.

**12.4 The Simulation Engine — what it actually does (`src/shared/simulation/simulation-engine.ts`), stated precisely so no one mistakes it for a real runtime:**
- Token estimate: `max(420, round(input.length * 2.8 + nodeCount * 180))` — pure string-length arithmetic, not a tokenizer.
- Latency: `round(620 + nodes.length * 115 + input.length * 1.4)` ms — synthetic.
- Cost: `round(tokens * 0.000018, 4)` USD — synthetic.
- "Needs" extraction: keyword match — contains `"crm"` (case-insensitive) → `["CRM integration","implementation timeline","summary accuracy"]`, else a generic fallback pair.
- "Risks" extraction: keyword match on `"стоим"`/`"cost"` → `["cost sensitivity"]`, else a generic fallback.
- `confidence` is a **hardcoded constant `0.86`**, never computed.
- Every run returns `status: "succeeded"` — there is no simulated failure path at all.
- Logs are emitted in node **array order**, not actual graph execution/topological order — do not use simulated logs to reason about real execution order.
- Node fields `promptId`, `modelId`, `temperature` are **completely unused** by the simulator — changing them in the Pipeline editor currently has zero effect on Playground output. This is the single most important limitation to communicate to anyone who might assume Playground reflects pipeline configuration.

**12.5 Target real pipeline (bridging to §3.3).** When the real runtime replaces the simulator, the first reference pipeline SHOULD be the call-analysis to-be pipeline from `pdf-notes.txt`: `Input (audio) → Router (КЦ/Agent) → LLM (transcription enhancement, narrow prompt) → Validation (deterministic regex normalization — NOT LLM) → LLM (structured summary, schema in §3.3) → Human Review (low-confidence gate) → Store → Output`. This maps directly onto the 10 existing `Node` types with no domain-model changes needed — confirming the entity design already anticipated something like this case, even without an explicit doc saying so.

**Anti-patterns:**
- Treating Playground output as evidence about pipeline correctness — it currently reflects only input string length and two keyword checks, nothing about the actual pipeline graph.
- Adding real-looking cost/latency/confidence numbers to the simulator to "make demos better" — that manufactures false evidence (violates §2 principle 5, R-010).
- Parsing `Edge.condition.expression` as if a grammar for it exists — it doesn't yet; define one first (§14) if you need to execute it.

**Definition of Done:** anyone building against "the pipeline" can state, without checking code, whether they mean the domain model (real, load-bearing) or the Simulation Engine (fake, demo-only, ignores node config).
...
**14.1 Current state.** "Pipeline DSL" today is nothing more than: the `Node`/`Edge` Zod schemas (structural DSL for the graph shape) plus free-text `metadata: Record<string,string>` per node (e.g. the demo pipeline uses `metadata.threshold = "0.72"`, `metadata.sla = "15m"`, `metadata.retention = "90d"`, `metadata.format = "json"` — all untyped strings, not a schema) plus a free-text `condition.expression` string on `Edge` with **no parser, no grammar, no evaluator anywhere in the codebase.**

**14.2 Mandatory rule before building a DSL.** Do not invent a bespoke expression language for `condition.expression` without first checking whether a structured alternative (e.g., a small typed condition object: `{ field, operator, value }`) would cover the actual use cases seen in `demo-data.ts` (a numeric confidence threshold comparison) — per §2 principle 6 (simplicity first), prefer the smallest structure that covers real cases over a general-purpose language.

**14.3 Structured output schema requirement.** Any future DSL for `llm`/`agent` node output MUST support the structured shape already proven necessary by the real business case (§3.3): `{ кто, тип_контакта, потребность, бюджет, срок, способ_оплаты, вопросы[], статус, действие, цитаты[] }`. Note the `цитаты[]` (quotes/citations) field — the to-be pipeline requires every key field to be grounded in a literal quote from the source transcript, which is a controllable, testable form of hallucination mitigation (`skills/senior-ai-solution-architect/AI_PATTERNS.md` pattern `structured-outputs`, and `ANTI_PATTERNS.md` AP-004). Any DSL for AI node output schemas should make citation/grounding a first-class, not bolted-on, concept.
...
**Lifecycle** (spec, not yet enforced in code — `Prompt` has no `status` field today, see §9.4): `Create → AI Draft → Review → Version → Evaluation → Approval → Pipeline Integration → Production`.

**[CANONICAL DECISION — gap to close].** Because `Prompt` currently has no lifecycle status field, any prompt referenced by `Node.promptId` is implicitly treated as "production" the moment it's wired into a pipeline — there is no code-level gate today preventing an unreviewed prompt from being used. Until a `status` field and gate are added to `Prompt` (an additive schema change, §9.5 DDD-1), enforce this manually: do not wire a `promptId` into a non-draft `Pipeline` without a corresponding `Review` record (`targetType` would need to be extended to include `"prompt"` — currently `ReviewTargetType` is `"product"|"architecture"|"pipeline"|"run"|"project"` and does **not** include prompts, a concrete documented gap).

**Definition of Done:** a change that introduces a new production-bound prompt either extends `Prompt`/`ReviewTargetType` to support real lifecycle gating, or is explicitly flagged as using the manual-review workaround above with a linked technical-debt note (§63).
...
**31.10 Code-vs-doc drift** (not a doc-vs-doc conflict, but equally important): `docs/ux/SCREEN_PRODUCT.md` and `SCREEN_ARCHITECTURE.md` describe fully interactive tabbed sections (Idea/Discovery/Frameworks/PRD/Review, etc.), but `product-screen.tsx` and `architecture-screen.tsx` render a `Tabs`/`Tab` row that is **decorative only** — clicking a non-first tab does nothing; all content renders at once below in fixed order, not gated by the active tab. Treat the UX doc as describing *intent*, and this discrepancy as open technical debt (§63), not as a doc error to silently "correct" by rewriting the doc down to match the simpler code.
...
- **J-3.** `Node.metadata` and similar free-form `Record<string,string>` fields are a last resort, not a default — prefer adding a typed optional field to the schema over stuffing a new concept into `metadata` as an untyped string (§14.1 already flags this as existing debt for `condition.expression` and `metadata` values like `threshold`/`sla`/`retention`/`format`).
