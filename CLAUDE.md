...
12. **Pipeline canvas renders 0 edges** — NEW, found 2026-07-03 while verifying task 4.2 in-browser, confirmed present in the pre-roadmap baseline too (not a regression introduced by this Roadmap). `src/features/mvp/screens/pipeline-screen.tsx`'s `<ReactFlow>` sits inside `<div className="min-w-0 flex-1">` with no explicit height; the browser console shows `[React Flow]: The parent container needs a width and a height to render the graph` repeatedly. All 8 demo nodes render (visually malformed, no correct transform) but `.react-flow__edge` count is 0 — none of the 8 demo edges render at all. Likely fix: give the wrapping div an explicit height (e.g. `h-full`, matching the sibling `<div className="flex h-[calc(100vh-56px)] min-h-0 flex-col">` pattern already used one level up). Not fixed here — unrelated to Edge.condition typing (§14.2) and deserves its own focused pass with an in-browser-verified fix.
