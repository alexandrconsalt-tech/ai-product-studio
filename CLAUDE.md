...
## Addendum (2026-07-26) — second, fully isolated product: «Анализ звонков v2»

**Task.** Add a second call-summary product/pipeline (5 stages: Facts & Quotes -> Needs -> Outcome & Next Step -> Summary Generator -> Summary Quality Gate) without touching the original product's stages, prompts, JSON contracts, reports, or run history in any way, while reusing shared infra (UI, storage, model calls, run history, dashboards).

**Read `docs/NEW_SUMMARY_PIPELINE_SPEC.md` in full before touching this product** -- it is the authoritative spec for this product's goals, JSON contracts, summary-format rules, and quality-gate rubric, the same way §3.3/`pdf-notes.txt` is for the original call-transcription pipeline. Do not change the original pipeline (`public/pipeline-lab-v3.html`) based on anything in that document.

**What was added, all net-new files:** `src/features/call-summary-pipeline/{lib,screens,model}/*` -- an independent React/TS engine following the exact architectural pattern of the repo's other second-pipeline precedent, `AdCopyTestBenchPanel`/`ad-copy-test-bench.ts` (data-driven stage config, `callModelByName`/`parseJsonResponse` from `browser-direct-provider.ts`, own Zod schemas, own JSON-repair pass duplicated rather than imported so this task never has a reason to touch `ad-copy-test-bench.ts` either). A single deterministic function, `computeQualityDecision()`, computes the quality report from raw 0-4 criterion scores for *both* the AI judge's output and a saved human evaluation -- the mechanism that guarantees "AI и человек используют одинаковые критерии/шкалу/веса/правила расчёта" per the spec. `docs/NEW_SUMMARY_PIPELINE_SPEC.md` was added as this product's own PRD-equivalent.

**What was additively touched (existing files, no removed/changed behavior for other products):**
- `src/features/mvp/screens/playground-screen.tsx` -- one new `pipeline.id === CALL_SUMMARY_PIPELINE_ID` branch (same shape as the existing `AD_COPY_PIPELINE_ID` branch) routing to `<CallSummaryPipelinePanel/>`, plus one new `handleCallSummaryRunComplete` callback recording into `usePlaygroundTestRunStore`. The `!pipeline` and `AD_COPY_PIPELINE_ID` branches, and the generic Pipeline Executor path, are untouched.
- `src/entities/PlaygroundTestRun/model/{schema,types}.ts` -- one new literal, `"call-summary-pipeline"`, added to the `PlaygroundTestRunSource` union/enum. The three existing literals are untouched.
- `src/shared/repositories/local-storage-repository.ts` -- one new function, `withCallSummaryPipelineModule()`, following the exact same unconditional-injection pattern as the existing `withTranscriptionSummaryModule()` (needed because a plain `demo-data.ts` fixture would not reach browsers with an already-persisted snapshot, and because `RETIRED_DEMO_PROJECT_IDS` already shows what happens to a fixture-only second product -- see `project_ad_copy_generation`, cascade-deleted on every load). Wired into the same 4 call sites `withTranscriptionSummaryModule()` already was, composed around it (`withCallSummaryPipelineModule(withTranscriptionSummaryModule(...))`), so every existing product's seeding/pruning behavior is unchanged.

**Why the React/TS-engine architecture, not a second HTML file.** `pipeline-lab-v3.html` derives a hardcoded behavior flag (`IS_TRANSCRIPTION_SUMMARY_MODULE`) from an exact product-name regex match; any other product reusing that file (even via its `preset=blank` mode) silently falls into unrelated legacy code paths from the file's pre-2026-07 history, not a clean slate -- so genuine isolation inside that file was never actually available. `AdCopyTestBenchPanel` is this repo's own proven precedent for a second, zero-shared-code pipeline engine living in the same Playground.

**Dashboard/history integration, deliberately with zero changes to `dashboard-screen.tsx`.** `PlaygroundTestRun`'s generic fields (`costUsd`, `durationMs`, `confidence`, `qualityScore`, `decision`, `status`, `report: z.unknown()`) already drive the Dashboard's product filter, stat tiles, and line charts without any dashboard code caring which `source` produced them -- confirmed by reading `dashboard-screen.tsx`/`playground-dashboard-analytics.ts` before writing a single line. The five old-pipeline-specific run-history columns (`truth_check` etc.) simply render "—" for this product's runs, an honest degrade rather than a crash. What is genuinely out of scope, called out explicitly to the user rather than silently skipped: `/reports`+`/review` (`src/features/summary-review/*`) is not duplicated for this product (it is hardcoded to the original pipeline's 5 judge-field vocabulary and has no product-scoping at all); this product's human evaluation + AI-vs-human comparison instead live directly inside its own Playground panel.

**What was NOT changed.** `public/pipeline-lab-v3.html` -- zero bytes touched, confirmed via `git diff` before this addendum was written. `src/features/mvp/lib/ad-copy-test-bench.ts` / `ad-copy-test-bench-panel.tsx` -- zero bytes touched (JSON-repair logic was duplicated into the new engine file rather than imported/exported from there). `src/features/summary-review/*`, `src/app/reports/*` -- untouched, out of scope per above. `demo-data.ts` -- untouched; this product's fixtures live entirely in the new `withCallSummaryPipelineModule()` injection function, matching the existing `withTranscriptionSummaryModule()` precedent rather than adding another entry to `RETIRED_DEMO_PROJECT_IDS`'s neighbor list.

**Verified.** `npx tsc --noEmit` clean (one pre-existing, unrelated error in `playground-test-run-store.test.ts` confirmed present on `main` before this branch via `git stash`). `npm run lint` clean. `npx vitest run` 286/286 (was 280 on `main`; +6 new `computeQualityDecision` unit tests covering equal-20%-weight scoring, all four threshold bands, the critical-issue 60% cap, and AI/human input-shape symmetry; the one pre-existing `local-storage-repository.test.ts` assertion that hardcoded "0 pipelines after pruning" was updated to expect the one pipeline this addendum's injection now always adds). `npm run build` succeeds. `npm run test:smoke` 22/22 (both `chromium` and `mobile-chromium` projects) -- required one fix during verification: `withCallSummaryPipelineModule()` originally prepended its project like `withTranscriptionSummaryModule()` does, which silently made the new product the effective default-selected project and broke `dashboard-run-history.spec.ts`'s implicit reliance on `project_transcription_summary_module` staying first; fixed by appending instead of prepending. Live-browser verification (own `next dev` instance pointed at this worktree, not any other locally-running copy of this repo): selected the new product in Playground, ran the pipeline with no BYOK key configured, confirmed graceful `TECHNICAL_ERROR` handling end-to-end (stage 1 shows "Не задан API-ключ OpenAI", the run bar shows a clean technical-error alert instead of a crash, stages 2-5 correctly never attempt), and confirmed Dashboard's product filter lists the new product separately and renders its one recorded run (cost/duration/status) with zero changes to `dashboard-screen.tsx`. A real end-to-end run with an actual model call was not performed (no API key available in the verification environment) -- the JSON-contract/prompt layer is exercised at the type level (Zod schemas) and via the `computeQualityDecision` unit tests, but not against real model output; the user should run one real transcript through it after a BYOK key is configured. Caught and fixed one real bug during this verification that unit tests didn't catch: a temporal-dead-zone `ReferenceError` in `runCallSummaryPipeline`'s early-return path (`retryCount`/`aiQualityReport` were `let`-declared after the first possible early `return finalize(...)`) -- moved both declarations above stage 1.
