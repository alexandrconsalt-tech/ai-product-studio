...
## Addendum (2026-07-06, third same-day pass) — Ad Copy Generation brought to production quality: same 6-node architecture, better prompts/data/algorithms only

**Purpose.** A production-quality pass explicitly scoped to *not* touch the pipeline's shape: the same 6-node chain (Validation code → Benefits LLM → Storage → Generation LLM → Checker LLM → Publish code), zero new stages, zero new LLM calls, zero routing/reflection/self-critique/debate. Reference material supplied: a diagram confirming the existing architecture was already correct, a document of real successful Avito/ЦИАН-style listings (style reference), and the platform's actual current single-prompt production system prompt (used as the source for its user-settings handling logic and its mandatory legal-disclaimer requirement, both carried into the new multi-stage prompts).

- **JSON Repair (new, `repairAndParseJson()` in `ad-copy-test-bench.ts`).** Runs on every `llm`/`check` stage response before schema validation: normalizes smart quotes, strips trailing commas, escapes raw control characters left inside string literals (the single most common cause of an otherwise-correct LLM response failing `JSON.parse`), and closes brackets left open by a truncated response — implemented as three small char-by-char scanners (`fixControlCharsInsideStrings`, `closeUnbalancedBrackets`), never a blind regex that could corrupt content outside string literals. A response that needed repair but came out complete and correct is still a successful stage (`status: "ok"`), not a warning — matches the requirement that repair-without-data-loss counts as success. Genuinely unparseable text (after repair) is reported honestly as such, never guessed at.
- **Benefits Agent rewritten to produce ready-to-use benefits, not characteristics.** `BENEFITS_PROMPT` now gives the model worked examples ("Две лоджии" → "Две лоджии позволяют организовать дополнительную зону отдыха и хранения") and requires every `advantages` entry to be a finished sentence about the reader's benefit. `AdCopyBenefitsSchema` (`ad-copy-benefits.ts`) enforces this at the schema level too — `.min(10)` per entry (a bare characteristic like "Ремонт" fails outright) and `.min(3)` entries total.
- **Simplified data contract end to end (no duplication).** `AdCopyBenefitsSchema` dropped `strengths` (redundant with `advantages`/`selling_points` — three overlapping "list of good things" arrays with no real boundary) and `target_audience` (redundant with the already-available `user_settings.target_audience` input — no reason to copy it back out as a second list). `ctx.stored` (`fnStorage`) now nests exactly `{property, user_settings, benefits: {usp, advantages, selling_points}}` — matching the explicit instruction to pass the Generator only `property`/`user_settings`/`benefits`. `AdCopyTestBenchPanel`'s `CONFIG_VERSION` bumped to `3` (from `2`) since this changes `ctx.stored`'s shape — a stage list saved under the old flat shape would otherwise reference now-nonexistent fields silently.
- **Generation prompt rewritten against real Avito/ЦИАН copywriting practice**, drawing directly from the supplied reference documents: sell the lived experience (one strong hook built around 1-2 key benefits, feelings and scenarios rather than a spec dump), natural non-boilerplate subheadings only when `user_settings.structure` asks for structure, never naming the target audience directly, avoiding AI-sounding boilerplate and repeated ideas, and a `user_settings`-priority section (style/focus/length/emoji) carried over from the platform's own existing single-prompt system prompt. The **mandatory legal disclaimer by deal type** (verbatim text, from the same existing production prompt) is requested in the prompt *and* separately guaranteed deterministically: `ensureDisclaimer()` in `fnSaveAd` appends the correct sale/rent disclaimer to the final `description` if it isn't already present (a simple substring check, so it never duplicates one the model already included) — this makes a business-critical legal requirement immune to the model paraphrasing it away, without adding a stage or a call.
- **Checker strengthened with 4 new checks** it's uniquely suited to judge: `ai_cliches_ok` (generic AI-sounding boilerplate -- "уникальное предложение", "лучшее на рынке" -- a real quality defect distinct from a *regulated* word like "гарантия"), `user_settings_ok` and `platform_requirements_ok` (semantic judgment calls a deterministic length/character check cannot make), plus its existing `issues` array (each item now individually surfaced as its own failed check in the stage's report, not just buried in the raw JSON, per the "подробный отчёт" requirement).
- **Confidence rebuilt as 6 named components** matching the audit's explicit list — validation (15) / JSON correctness (10) / schema match (15) / platform requirements (20, combining the deterministic length/character check with the Checker's own semantic judgment when it ran) / Checker content (25, only when the Checker ran) / user-settings match (15, only when the Checker ran) — replacing the previous pass's more ad-hoc structural checks. The 2 Checker-only components are excluded from the denominator (not scored as a failure) when the Checker didn't run, preserving the "an absent optional cross-check never itself caps confidence" principle from the prior audit.
- **Test-bench UI now surfaces the 4 diagnostics requested**: `AdCopyStageReport` gained `rawResponse`/`jsonRepaired`/`repairedJson`; `StageReportBody` (`ad-copy-test-bench-panel.tsx`) renders a collapsible "RAW-ответ модели" section (with a "JSON исправлен" badge and the repaired text when repair happened) directly above the existing "Итоговый валидный JSON" -- so a wrong or malformed model response can be diagnosed without leaving the page.

**A real bug found live while verifying this end-to-end in-browser (not shipped)**: my own verification mock matched the Benefits-stage response to *any* prompt containing the substring `"usp"` -- which the Generation prompt itself also contains, since it embeds `{{ctx.stored.benefits}}` (a JSON dump including `"usp":...`) as context. The Generation stage correctly rejected the wrong-shaped mocked response via the existing per-stage contract check (`STAGE_OUTPUT_SCHEMAS`) rather than silently accepting it -- a live demonstration of that feature working, not a product defect. Fixed by matching on each prompt's own distinguishing opening text instead of a JSON key that appears in more than one prompt's rendered output.

**What was NOT changed.** The 6-node architecture, `AdCopySchema`/`AdCopyQualityCheckSchema`'s existing `title`/`description`/`cta` and boolean-per-check shapes (only additive fields), the domain-executor path and its seed prompts, and the platform-facing `{property, user_settings}` → final-ad "external API" are all unchanged, per explicit instruction. Verified: `npm run lint` clean, `npx vitest run` 231/231 passing (11 new/updated tests: 5 for `repairAndParseJson` directly, plus contract/confidence fixture updates for the simplified Benefits shape and the expanded Checker shape), `npm run build` succeeds, and a full manual in-browser run (mocked OpenAI responses, only an OpenAI key configured, one deliberately malformed trailing-comma Generation response) produced: a live JSON Repair ("ответ модели содержал невалидный JSON — автоматически восстановлен без потери данных", status still "успешно"), Confidence 100%, a `Quality Gate` "SAVE" decision, and a final ad whose description carried the mandatory sale disclaimer exactly once (not duplicated, since the mocked response already included it).
...
## Addendum (2026-07-06, seventh same-day change) — Pipeline Lab V8.1: transcript-aware summary, extraction fixes, hallucination-safe Quality Gate

**Purpose.** A quality regression report on "Транскрибация и саммари" (AI Call Analysis): the summary (step 7) lost key call detail because it never received the transcript, steps 3/5 mis-extracted a listing price as the client's own budget and a viewing date as the deal timeline, and Quality Gate could `AUTO_SAVE` a summary the Checker itself had flagged as containing hallucinations. Six changes (T1-T6, one commit each), against `public/pipeline-lab-v3.html`'s `defaultPipeline()`/`CODE_FUNCS.gate` -- no architecture change, per the request's own constraint.

- **T1+T2 (one commit).** Step 7's prompt (`{{transcript}}` was entirely absent) now receives the full transcript and was rewritten end to end: transcript-as-primary-source rules, explicit role handling (Оператор excluded from the summary itself but its context retained), a structured extraction checklist (questions asked/answered, money with an explicit listing-price-vs-budget distinction, deal timeline vs. viewing-date distinction, access restrictions, status vocabulary), call-scenario branching (не состоялся/недостаточно информации/нецелевой/технический/полноценный), and worked examples. Output contract (`{"summary": "..."}`, `outKey: "summary"`) and steps 8-10 integration unchanged, per the request's explicit constraint.
- **T3.** Step 3's (Извлечение фактов) prompt gained an explicit rule: a sum quoted only to identify/confirm a listing's price (including via the operator) is the property's price, not the client's stated budget -- `budget` stays `null` in that case. Output schema (validated by step 4) unchanged.
- **T4.** Step 5's (Определение потребностей) prompt gained the parallel rule for `timeline`: an agreed viewing/call date is not a deal timeframe -- `timeline` stays `"не определено"` unless the client stated an actual purchase/deal timeframe. Output schema unchanged.
- **T5.** `CODE_FUNCS.gate` gained a hard rule layered on top of the existing weighted score (`evidence 25% / validation 15% / schema 15% / fields 15% / check 30%`, thresholds `AUTO_SAVE ≥0.92 / AUTO_SAVE + лог ≥0.80 / RETRY ≥0.65 / else FALLBACK`, all unchanged): if the Checker (step 8) reported any `hallucinations`, the decision is capped at a new `REVIEW` value even when the weighted score alone reached `AUTO_SAVE`/`AUTO_SAVE + лог` tier -- `RETRY`/`FALLBACK` outcomes (already stricter) are left alone. The override reason ("Причина: галлюцинации: N") is surfaced as its own check line in the stage's report. `CODE_FUNCS.crm`'s existing `saved = decision==='AUTO_SAVE'||decision==='AUTO_SAVE + лог'` check needed no change -- `REVIEW` already falls through to "not saved, sent to review" the same way `RETRY`/`FALLBACK` already did.
- **T6.** The Checker's (step 8) prompt already received the transcript (this one, unlike step 7, already had `{{transcript}}`); gained two explicit exceptions matching T2's new Summary Agent output so the Checker doesn't flag its own intended behavior as an error: the dictionary-only "Статус: ..." line (each descriptor confirmed by the transcript) and an amount tagged "(цена в объявлении)" are no longer treated as a hallucination or leaked PII.
- **Config versioning (prerequisite, own commit).** `loadPipelineConfig()` previously let *any* saved-in-this-browser stage list permanently override `defaultPipeline()` with no way to tell it was stale -- meaning none of the above would reach a browser that had ever clicked "💾 Сохранить" once. Wrapped the saved config with a `PIPELINE_VERSION` stamp (now `2`); a config saved under a different (or legacy unversioned/bare-array) version is discarded, falling back to the current, fixed defaults. Same pattern as `CONFIG_VERSION` in `ad-copy-test-bench-panel.tsx`.

**A gap flagged, not silently worked around.** The task's own reference call ("Андрей, Веденская, 24 к.1") is described only via a testing checklist -- neither the literal transcript nor this exact call is present anywhere in the repo, and the tool's built-in "Вставить пример" is a different, shorter sample (Татьяна/новостройка), left unchanged since nothing asked for it to be replaced. Verification used a reconstructed transcript built to satisfy every checklist item (price-in-listing notation, document/reason-for-sale Q&A, a relative's-consent restriction, a viewing-time agreement) rather than the original recording -- reported here explicitly rather than presented as the literal source material.

**What was NOT changed.** Steps 1, 2, 4, 6, 9 (structure/thresholds), 10, and every output schema (`store`'s `required`/`citationFields`, the Checker's JSON fields) are untouched -- only the prompt *text* of steps 3/5/7/8 and gate()'s decision logic changed, per the request's constraints. Verified: `node --check` on the extracted inline script, `npm run lint`/`npx vitest run` 236/236/`npm run build` all pass. Manually confirmed in-browser end to end (mocked LLM responses, since no real API key is available in this environment): budget correctly stayed `null` and timeline `"не определено"` on the reconstructed reference call; a clean run (no hallucinations) produced `AUTO_SAVE` at confidence 0.99; an otherwise-high-confidence run (0.89, would have been `AUTO_SAVE + лог`) with 2 mocked hallucinations was correctly forced to `REVIEW` with the reason line visible; and a short/off-topic dialogue regression-tested cleanly to the "Недостаточно информации" scenario without the pipeline erroring. Actual LLM adherence to the rewritten prompts' wording can only be confirmed with a real Anthropic/OpenAI key in production -- recommend a real run against a live call once deployed.
...
## Addendum (2026-07-06, tenth same-day change) — Pipeline Lab V8.4: user configs survive deploys and runs

**Bug reported.** Manual edits made in the embedded Pipeline Lab stage editor (prompt/name/etc.) appeared to save and survive a page refresh, but a subsequent run could look like it used reset/default settings. The underlying persistence rule was also wrong for the product expectation: saved configs were rejected whenever `PIPELINE_VERSION` changed, meaning deploys that bumped the version could silently discard real user edits.

**Changed in `public/pipeline-lab-v3.html`.** `PIPELINE_VERSION` is now migration metadata only, not an invalidation key. `loadPipelineConfig()` accepts any saved stage list (including legacy bare-array saves) and re-saves it under the current version on the next save/run, so user edits remain authoritative across future deploys. `runPipeline()` now calls `syncPipelineFromEditor()` and `savePipelineConfig()` before executing stages, and the per-stage Save button also syncs the live form first. This makes the visual editor state, localStorage state, run input, and downloaded/dashboard report use the same stage configuration.

**Changed in `src/features/mvp/screens/ad-copy-test-bench-panel.tsx`.** The Ad Copy stage editor now follows the same durable-config rule: `CONFIG_VERSION` is migration metadata only, older saved stage lists are loaded instead of discarded, every field edit immediately persists the full stage list, structural edits persist, and `handleRun()` saves the exact stage list it is about to execute.

## Addendum (2026-07-06, eleventh same-day change) — Pipeline Lab V8.5: specialized Russian Summary quality judges

**Reason.** The previous call-summary quality architecture used one broad "Проверка саммари" step before a generic Quality Gate. Product evaluation now requires explainable Summary quality assessment through specialized Russian-language judges: truthfulness, critical facts, usefulness for the next agent, agreements/next step, and presentation/rules. The key product principle is that Summary is not a full call retelling; it should contain only facts needed for the next agent to continue work.

**Changed in `public/pipeline-lab-v3.html`.** The default pipeline keeps steps 1-7 unchanged, then replaces the single summary checker with five checker stages: `truth_check`, `critical_facts_check`, `context_utility_check`, `action_check`, and `presentation_check`. A new code step `summary_quality_gate` calculates `summary_quality_score` using weights 30/25/20/15/10 and returns one of `AUTO_SAVE`, `RETRY`, or `MANUAL_REVIEW`. CRM save now reads the new Summary Quality Gate and saves only on `AUTO_SAVE`.

**Robustness and compatibility.** `PIPELINE_VERSION` was bumped to `6`, but the V8.4 durable-config rule remains: saved user configs are migrated, not discarded. Old saved chains containing `summary_check` + `gate` are upgraded in place by removing the old summary checker/gate and inserting the five new judges plus `Summary Quality Gate` before CRM. If a checker API call fails or returns invalid JSON, that checker becomes a fail result with `score: 0`; the overall gate still runs and surfaces the failure as a critical issue. Claude checker steps can fall back to GPT-5 mini when Anthropic is unavailable and an OpenAI key exists.

## Addendum (2026-07-07) — Pipeline Lab V8.6: visible judge criteria and dashboard quality analytics

**Reason.** After V8.5, Summary quality was explainable in JSON but still too compressed in the UI: stage cards did not consistently show a 0-100 score, judge criteria were not visible, and Dashboard could not aggregate judge/stage quality or decision reasons. A real call also showed that the Summary Agent could fall back to "client asked -- agent answered" phrasing, which is not the desired CRM working-summary style.

**Changed in `public/pipeline-lab-v3.html`.** Every stage report now gets a normalized `meta` block with `status`, `score`, `confidence`, `durationMs`, `criteria`, `issues`, and `recommendations`. Stage cards display "Оценка: XX%" and an expanded "Критерии оценки" block with pass/warning/fail indicators. Summary judge outputs now include criteria. The Summary Quality Gate adds typed warning details, deduplicates recommendations, filters forbidden "add phone/address/price/object-card data" recommendations, shows only top-3 recommendations in the main UI, and keeps the full list in JSON. Downloaded/postMessage reports now include `stageReports` for dashboard aggregation.

**Prompt and policy updates.** The Summary Agent prompt now explicitly requires final working facts instead of question-answer retelling ("Спросил/уточняла X -- агент ответил Y" is forbidden). The Presentation Judge now distinguishes forbidden exact object/card data from useful negotiation context: "готов к быстрому выкупу", "просит торг", "обсуждает комиссию" are allowed when written without exact sums.

**Changed in Dashboard.** `computeDashboardStats()` now extracts average stage scores, judge scores, decision shares (`AUTO_SAVE`/`MANUAL_REVIEW`/`RETRY`), top MANUAL_REVIEW/RETRY reasons, and average time/cost/score per stage from saved `stageReports`. `dashboard-screen.tsx` renders these aggregates and expands run history with Summary Quality Score, decision, five judge scores, failed stage, main issue, final summary, stage scores, and full JSON. Old runs without `stageReports` remain readable; they simply do not contribute to the new detailed aggregates.

## Addendum (2026-07-07, second same-day change) — Pipeline Lab V8.7: shorter factual Summary and compact Gate report

**Reason.** A real Summary quality run still produced too much dialogue-retelling language ("спросила", "уточняла", "агент ответил") and the Summary Quality Gate main UI was too verbose. Product direction is now stricter: Summary is a short CRM fact extract, not a transcript summary or question-answer recap.

**Changed in `public/pipeline-lab-v3.html`.** The Summary Agent prompt now targets 2-3 short factual sentences (goal 500 chars, hard max 700), explicitly bans dialogue-retelling terms and `клиент X -- агент Y` constructions, forbids "готов к сделке" unless there is an accepted offer/price/next deal step, and requires `Следующий шаг: не согласован` when no explicit next step exists. Action Check treats that phrase as correct absence-of-agreement handling, while invented next steps become errors.

**Gate/UI behavior.** Presentation Check now has a "Факты вместо пересказа" criterion and fails/caps score when dialogue-retelling phrases appear repeatedly. Summary Quality Gate normalizes long judge outputs into a compact main card: score, decision, main reason, up to 3 key problems, and up to 3 actions. Full details remain available only in the JSON disclosure.

## Addendum (2026-07-07, third same-day change) — Pipeline Lab V9.1: AI Summary stabilization pass (real-run diagnosis, not a new architecture)

**Reason.** A real Pipeline Lab run on the 14-stage/5-judge architecture (V8.5-V8.7) scored 85.5 and landed on `MANUAL_REVIEW` instead of the target `AUTO_SAVE`. The run's own JSON pinpointed six concrete causes: (1) Outcome's `agreements` had no definition of "agreement", so fact-confirmations ("квартира продаётся", "номер актуален") were miscounted as agreements and penalized by Action Check for their absence; (2) the three judges most focused on next-agent usefulness (Critical Facts, Utility, Action) had no shared knowledge that a CRM card already exists, so they demanded fields (name, address, price) the Summary Agent is explicitly forbidden from writing; (3) Presentation had no explicit client/agent-name rule; (4) the Quality Gate's own deterministic override for "invented next step" could fire even when the Action judge's own `is_next_step_reflected` field said the next step *was* correctly reflected -- a genuine phantom-`key_problems` bug, not a data issue; (5) `summary_quality_score`'s weights (30/25/20/15/10) didn't match the product's stated Quality Framework v1 distribution; (6) Fact/Need Agent prompts let object-card fields (full address, exact area/floor/price) leak into `city_or_district`/`property_requirements`, polluting the Summary Agent's own inputs.

**Changed in `public/pipeline-lab-v3.html`** (`PIPELINE_VERSION` 6 → 7):
- **Outcome Agent (step 6):** added an explicit "agreements (V9)" rule distinguishing a mutual future-action commitment from a fact confirmation, a service refusal, an interest, or a contact exchange; added an additive `next_step_owner` field (`store`/`gate`/`crm` read `ctx.outcome` by specific known keys only, so this doesn't require any schema change elsewhere).
- **Fact Agent (step 3):** added a `city_or_district` rule (city/district only, never the object's street address) and two additive fields, `intent` and `client_availability` (verified `store()`'s only hard checks are `required:['client_name']` and the `source_of_funds` enum -- unrelated fields pass through untouched).
- **Need Agent (step 5):** added a `property_requirements` rule banning object-card duplication (address/rooms/area/floor/price), requiring only the client's own stated requirements for the object being searched for.
- **Summary Agent (step 7):** kept every already-shipped protection from V8.1-V8.7 (transcript-as-truth-arbiter, CRM-card no-duplication, "готов к сделке" ban, dialogue-retelling ban, the `Статус:`/`Договорились:`/`Следующий шаг:` structure the 5 judges are calibrated against) and merged in only the genuinely new V9.1 requirements: length target raised 500 → 600 (hard cap still 700), and an optional 4th block of 2-4 self-contained bullet-point markers for critical facts with verbatim citations in quotes when useful, plus a lightweight internal self-check list. **This was a deliberate merge, not the literal replacement text originally drafted** -- the draft's proposed structure dropped the `Статус:` line entirely and restructured the format into 3 plain paragraphs, which would have desynced the Summary format from what Presentation/Action are calibrated to expect; merging preserves both the already-tested protections and the new V9.1 additions.
- **Critical Facts / Utility / Action judges (steps 9-11):** all three gained an identical "ВАЖНЫЙ КОНТЕКСТ — КАРТОЧКА CRM" block up front, stating explicitly that name/phone/address/deal-type/property-type/rooms/area/floor/price already live in the CRM card and must never be scored as missing, and that a fact-confirmation ("квартира продаётся") is not an agreement or a critical fact. Utility's main question was reworded to explicitly reference the CRM card. Critical Facts' "не считать критически важными" list gained three items: client name, listing-still-active confirmation, phone-number confirmation.
- **Presentation judge (step 12):** gained a new checklist item (numbered 13, since item 12 -- "написано итоговыми фактами, а не пересказом" -- already existed from V8.7 and the drafted "12." would have collided) requiring no client/agent name in the summary, plus a scoring rule capping score at 80 when a name is present.
- **Summary Quality Gate (`summaryQualityGate`):** reweighted to Truth 40 / Critical Facts 20 / Utility 23 / Action 15 / Presentation 2 (Quality Framework v1, weights now sourced by explicit product spec rather than the prior 30/25/20/15/10 split -- Presentation's low weight is intentional, since its hard failures are enforced deterministically, not through its score contribution). The `hasInventedNextStep` override now only fires when `action_check`'s own `is_next_step_reflected` field isn't already `true` -- fixing the phantom "Придуман следующий шаг" report. Two new deterministic safety nets run independent of any judge's self-report: an exact `>700`-character length check (ignoring the judge's own `length_chars` claim) and a phone-number regex scan (via the previously-unwired `detectForbiddenSummaryDetails()`, dating back to V8.2/V8.3) -- both feed into `retry`/`autoSave` directly. **`key_problems`/`main_reason` are now built strictly from the five judges' own structured JSON fields** (`critical_errors`, `missed_critical_facts`, `missing_for_next_agent` only when `status==='fail'`, `action_errors`, `forbidden_card_duplicates`/`pii_found`/`forbidden_phrases`) plus the two deterministic checks above -- no longer via the old `shortIssue()` free-text regex re-classification, which could label a warning as "PII" or "invented next step" without a matching structured field, independent of what the actual judge said.
- **Stage result transparency (Задача 8):** every `llm`/`check` stage report now also carries `actual_model`, `requested_model`, and `model_fallback` as first-class JSON fields (previously the vendor substitution was visible only as a text note inside the "Модель" metric), so judge-calibration analysis across runs can tell which vendor actually answered without string-parsing the metrics label.

**Gap flagged, not silently worked around.** The task's own reference call ("Илья / Предпортовая") is referenced only by a testing checklist, same as the prior V8.1 task's "Андрей / Веденская" -- neither the literal transcript nor this specific call exists anywhere in the repo, and the built-in "Вставить пример" sample is still the older Татьяна/новостройка call, unchanged (nothing in this task asked for it to be replaced). Verification used a reconstructed transcript built to satisfy every stated acceptance criterion (listing-price-vs-budget distinction, a real owner-lives-here/viewing-by-agreement fact, a genuine future callback agreement, decline of a mortgage consultation, availability due to not working, willingness to view other objects) rather than the original recording -- reported explicitly here, not presented as sourced fact.

**What was NOT changed.** Steps 1, 2, 4, 10, 13 (CRM save logic itself), the `store()`/`validate()` schemas, the domain-executor path, and the hidden legacy screens are untouched. Per V8.4, `PIPELINE_VERSION` remains migration metadata, not an invalidation key -- a browser with an already-migrated, previously-saved 14-stage config keeps its own (possibly hand-edited) stage text; only a browser with no saved config, or one still on the pre-V8.5 single-checker architecture, picks up these V9.1 prompts automatically via `migratePipelineConfig()`. Verified: `node --check` on the extracted inline script (syntax), `npm run lint`/`npx vitest run` 236/236/`npm run build` all pass (unaffected by this file, which has no dedicated automated test suite of its own), and a full manual in-browser run against the reconstructed reference transcript (mocked LLM responses, since no real API key is available in this environment) produced `summary_quality_score` 100 and `AUTO_SAVE` on a clean pass, and confirmed each of the three required negative scenarios (an injected hallucination, an 800-character summary, and a summary containing a phone number) independently forced `RETRY` via the mechanisms described above.

---

## Addendum (2026-07-07, fourth same-day change) — root cause found: "durable config" (V8.4) never refreshed prompt *content* for browsers already on the 5-judge architecture; per-stage `promptVersion` fix

**Bug reported.** A real post-deploy run ("Дмитрий" call) showed the exact same class of failure the V9/V9.1 work was supposed to have fixed by now: Fact Agent's output had no `intent`/`client_availability` fields (added in the V9.1 pass), and the Summary Agent (step 7) returned a plain-text refusal -- "Нет доступных данных: отсутствуют Facts JSON, Needs JSON и Call Outcome JSON..." -- instead of JSON, breaking `JSON.parse` and cascading into a low `summary_quality_score` (41.1, `RETRY`) even though the transcript clearly contained all the facts the refusal claimed were missing.

**Root cause, confirmed by direct inspection and a live reproduction.** `migratePipelineConfig()` (added V8.4, extended V8.5) has always had this shape: `if(hasSummaryQualityArchitecture(stages)) return stages;` -- an unconditional early return whenever the saved config already has the 5-judge architecture (Truth/Critical Facts/Utility/Action/Presentation + Summary Quality Gate). This check only verifies *structure* (do the right stage slots exist), never *content* (is the prompt text inside those slots current). Since this browser's saved config already had that structure (it had been running this pipeline across many sessions since V8.5), **every single prompt fix shipped from V8.5 onward -- including all of V9, V9.1, and the V9.1 stabilization pass -- silently never reached this browser.** The browser kept re-running whatever prompt text happened to be saved locally from an earlier session, while the shipped defaults in `public/pipeline-lab-v3.html` moved on underneath it. This was previously undetectable from the deployed code alone because `npm test`/`npm run build` have no coverage of this vanilla-JS file's runtime localStorage-migration behavior -- confirmed only by reproducing it live (below).

**Fix.** Two mechanisms, additive to the existing "durable config" design (still honors real user edits as authoritative, per V8.4's original intent):
1. Each `llm`/`check` stage in `defaultPipeline()`/`summaryQualityStages()` now carries an explicit `promptVersion:1` (facts, needs, outcome, summary, and all 5 judges).
2. `migratePipelineConfig()` now *always* runs a content-refresh pass after any structural migration (or immediately, if no structural migration was needed) -- for every stage matched by `outKey` against the current `defaultPipeline()`, if `stage.userEdited` is not `true` and `(stage.promptVersion||0) < def.promptVersion`, its `prompt`/`model` are overwritten from the current default and `stage.promptVersion` is bumped to match. `stage.userEdited` is a new flag, set by the stage editor's prompt/model input handlers in `renderStages()` (`public/pipeline-lab-v3.html`) the moment a person actually types into a stage -- so a real, intentional customization is still permanent and is never silently overwritten, matching the original V8.4 rationale; only a stage nobody ever touched, sitting on a stale version number, gets auto-upgraded. `PIPELINE_VERSION` bumped 8 → 9 (still migration metadata only, not an invalidation key, per V8.4).

**Verified live, not just by code inspection.** Extracted `defaultPipeline`/`migratePipelineConfig`/`hasSummaryQualityArchitecture`/`summaryQualityStages` into an isolated Node module and ran two scenarios: (1) a stale saved config with the 5-judge structure already present but an old/broken Facts prompt (missing `intent`) and a deliberately nonsensical Summary prompt -- `migratePipelineConfig()` correctly replaced both with the current defaults and stamped `promptVersion:1`; (2) the same stale config but with `facts.userEdited:true` -- the manual edit was correctly left untouched, `promptVersion` correctly left `undefined` (never touched). Repeated both scenarios end-to-end in a live browser preview (not just the isolated Node harness): saved a stale config to `localStorage['pipelineLabV3.pipelineConfig']`, reloaded the actual page, and confirmed via the page's own live `pipeline` variable that the stale Facts/Summary prompts were replaced with the current ones (`intent` present, correct `{{ctx.facts}}`/`{{ctx.needs}}`/`{{ctx.outcome}}` references) in the first scenario, and that the manual edit survived the reload unchanged in the second. `npm run lint` clean, `npx vitest run` 236/236, `npm run build` succeeds (this file has no dedicated automated test suite of its own, hence the isolated-module + live-browser verification above instead).

**What was NOT changed.** The actual prompt *text* of every stage is untouched by this fix -- this addendum is purely about making sure previously-shipped prompt text (from V9/V9.1/the stabilization pass) actually reaches browsers that got stuck on an older saved copy; it does not itself change what any prompt says. The structural-migration branch (5-judge insertion for pre-V8.5 configs) is unchanged.

---

## Addendum (2026-07-08) — stages 1-9 rework: per-field evidence/confidence schema, independent Fact/Need/Outcome Check agents, Conversation Store as the only input to Summary

**Task.** A detailed spec ("доработка AI Pipeline до этапа генерации Summary") asked for a rework of the pipeline's first nine stages so the segment before Summary reaches 90%+ quality: STT → Validation → Facts → **Fact Check** → Needs → **Need Check** → Outcome → **Outcome Check** → **Conversation Store** → Summary (stage 10 explicitly out of scope: "не трогать"). The three bolded stages and Conversation Store did not exist before this task -- Facts/Needs/Outcome previously fed Summary directly (with the V9/V9.1 deterministic-cleanup safety nets from earlier addenda). A companion instruction, given after the spec, required that *every* checking agent (not just Need Check) score using the Quality Framework v1 judge algorithm from `Спецификация системы оценки качества AI Summary (Quality Framework v1).docx` (already the basis of the 5 post-summary judges, confirmed by reading the doc: its 11-parameter weight table matches the shipped Truth 40/Critical Facts 20/Utility 23/Action 15/Presentation 2 split exactly) -- "was this fact discussed in the conversation at all? If no, don't evaluate; if yes, is it correctly reflected? Pass/fail" -- rather than penalizing an honest `null`/"не определено".

**Continuation, not a fresh start.** Work on this task had already begun in an earlier session and was interrupted mid-way (confirmed via `git diff` on session start: 317 uncommitted lines already rewriting `CODE_FUNCS.stt`/`CODE_FUNCS.validate` to the new quality-score/decision-state model). This session read that partial diff, verified it against the spec's exact formulas (`stt_quality_score` penalty weights, decision thresholds), found it correct, and continued from stage 3 onward.

**Corrections made to the spec itself** (per its own instruction to review and adjust before implementing):
1. **Decision-branch priority order** was unspecified everywhere a stage's PASS/PASS_WITH_WARNINGS/MANUAL_REVIEW/RETRY/FAIL/STOP conditions could overlap (e.g. score ≥0.75 satisfies both PASS_WITH_WARNINGS and, separately, a MANUAL_REVIEW condition). Resolved with one consistent rule applied to every decision stage: evaluate top-down, most-severe-first -- STOP/FAIL first, then RETRY/RETRY_STT, then MANUAL_REVIEW, then PASS, with PASS_WITH_WARNINGS as the catch-all fallback.
2. **Old "Единое хранилище" stage (`store` codeFn, position 4)** overlapped conceptually with the new Conversation Store (position 9) -- both validate/aggregate Facts. Resolved: the old flat `store` stage is removed from `defaultPipeline()`'s default order (its budget ×1000/×1 000 000 normalization heuristic moved into the new `conversationStore` codeFn); the `store` codeFn itself stays in `CODE_FUNCS`/`CODE_FN_LIST` only for backward compatibility with any hand-configured legacy stage.
3. **"Этап 10 не трогать" vs. the spec's own "Что передавать в Summary Agent" requirement** were in direct tension -- the latter explicitly demands Summary stop receiving raw `facts`/`needs`/`outcome`/transcript and receive only `conversation_store`. Resolved as the one disclosed, minimal exception: Summary's algorithm, style rules, forbidden-phrase list, output format, and worked examples are byte-identical; only the trailing "ВХОДНЫЕ ДАННЫЕ" template block changed from four `{{ctx.X}}`/`{{transcript}}` substitutions to one `{{ctx.conversation_store}}`, plus a matching two-sentence edit to the "ИСТОЧНИКИ ДАННЫХ" intro paragraph (transcript is no longer supplied, so "verify against the transcript" became "verify against each field's own `evidence`").
4. **Consequential, not explicitly requested, but required for internal consistency:** the four fact-referencing post-summary judges (Truth, Critical Facts, Context Utility, Action -- Presentation doesn't reference facts) had their input-wiring blocks changed from raw `{{ctx.facts}}/{{ctx.needs}}/{{ctx.outcome}}` to `{{ctx.conversation_store}}`, since Conversation Store is now the pipeline's declared single source of truth and raw facts/needs/outcome now hold pre-verification, possibly-rejected data. Judge scoring logic, weights, and examples are unchanged -- only which ctx key backs their "ground truth" section. `crm()` was updated the same way (`ctx.conversation_store.facts.client_name.value` etc., unwrapping the new nested shape into the flat card), with a fallback to the old flat `ctx.store`/`ctx.facts` path for any saved config that predates this rework and hasn't reloaded yet.

**What was built (9 stages, all now emit `{value, confidence, evidence, verification_status}` per field instead of flat values):**
- **Facts** (`outKey:'facts'`, GPT-5 mini): same 7 fields as before (client_name, intent, source_of_funds, city_or_district, budget, phone_mentioned, client_availability), rewrapped into the new schema; V8.1 budget rule and V9 city_or_district rule preserved, reworded for `.value`.
- **Fact Check** (`outKey:'fact_check'`, new, Claude Sonnet 4.6): independently verifies each fact against the transcript using the Quality Framework v1 "was it discussed? → is it reflected correctly?" algorithm; outputs `verified_facts`/`rejected_facts`/`fact_check_quality`.
- **Needs** (`outKey:'needs'`): `interested_in` dictionary corrected to `["Новостройки","Ипотека","Строительство"]` (dropping "Вторичка"/"Аренда", which the spec itself flagged as a bug -- those are transaction-type/market-segment values, not agency service offerings); added `market_type_interest: ["вторичка","новостройка"]` as its own field.
- **Need Check** (`outKey:'need_check'`, new, Claude): same Quality Framework v1 methodology, explicitly spelled out in the prompt per the follow-up instruction that *all* checking agents (not only this one) must use it; separately enforces the `interested_in`/`market_type_interest`/`source_of_funds`/`timeline` dictionaries.
- **Outcome** (`outKey:'outcome'`): `call_result` dictionary extended with "недостаточно информации"/"звонок не состоялся"; `next_step` gained `owner`/`deadline`; V9 agreements rule preserved verbatim.
- **Outcome Check** (`outKey:'outcome_check'`, new, Claude): same methodology, catches invented `next_step`/`agreements` not grounded in the transcript.
- **Conversation Store** (`outKey:'conversation_store'`, new, deterministic `codeFn:'conversationStore'`): keeps only `verified:true` fields with `confidence>=0.85` (except a conscious `null`/"не определено"/empty-array result, which is kept regardless of confidence -- a deliberate, verified absence is not the same as low-confidence noise); computes `overall_store_score = validation*0.2 + fact_check*0.25 + need_check*0.25 + outcome_check*0.3` and a PASS/PASS_WITH_WARNINGS/MANUAL_REVIEW/FAIL decision; keeps rejected data only in a `debug` sub-object, never used downstream.

**Cross-cutting additions:** every `llm`/`check` stage got a `promptVersion` bump (facts/needs/outcome/4 judges → 2, new stages → 1) so the existing per-stage content-refresh mechanism (previous addendum) auto-upgrades any already-saved browser config without a manual reset; `runStage()` now stamps `stage_version`/`prompt_version`/`model`/`timestamp` on every report centrally (not hand-written per prompt, since LLM output can't be trusted to include bookkeeping fields reliably); `migratePipelineConfig()` gained a second, independent structural-migration branch (`hasConversationStoreArchitecture()`/`insertMissingStageAfter/Before()`) that inserts the four new stages at the right position and removes the old flat `store` stage for *any* saved config missing them, regardless of whether that config already has the (unrelated) 5-judge summary architecture; the UI's 3-color status system (green/yellow/red) gained a 4th, orange `attn` state for `MANUAL_REVIEW` (new `--attn` CSS variable, `statusFromDecision()`, `badgeFor`/`badgeText`, `stageScoreFromOutput`, `metaStatusFromScore` all updated) so it doesn't collapse into the same yellow as PASS_WITH_WARNINGS. `PIPELINE_VERSION` bumped 9 → 10 (still migration metadata only, per V8.4).

**Verified live, not just by code inspection.** `npm run lint` clean, `npx vitest run` 236/236, `npm run build` succeeds. Beyond that (this file has no dedicated automated test suite of its own): (1) extracted `migratePipelineConfig`+helpers into an isolated Node module and confirmed both a "5-judge-architecture-but-pre-rework" config and a "very old pre-V8.5, single-checker" config each migrate to the identical correct 17-stage order (`stt, validation, facts, fact_check, needs, need_check, outcome, outcome_check, conversation_store, summary,` 5 judges, gate, `crm`); (2) ran the actual page in a live browser preview against a realistic reconstructed "Дмитрий" transcript (STT and Validation are deterministic code stages and ran for real, producing `PASS`/100% with the exact new quality-metric shape) and, since no API key is available in this environment, hand-verified the six new/rewritten LLM stage prompts and all downstream wiring by mocking realistic Fact/Need/Outcome-Check output directly into the page's live `ctx` and calling its own functions: `conversationStore()` correctly aggregated 7 verified facts / 5 needs / 3 outcome fields into `overall_store_score:0.947` → `PASS` (arithmetic checked by hand against the stated formula); a boundary case (one field `verified:true` but `confidence:0.5`) was correctly excluded while a `null`-valued field at `confidence:0.3` was correctly kept, and the resulting missing-intent state correctly forced `decision:'FAIL'`; `tmpl()` rendering of all 6 new/changed prompts and the Summary prompt produced no leftover `{{...}}` placeholders and confirmed the raw transcript text does **not** appear anywhere in the rendered Summary prompt (only `conversation_store`'s JSON does); `crm()` correctly unwrapped `conversation_store`'s nested `.value` fields into the expected flat CRM card.

**What was NOT changed.** Stage 10's algorithm, style rules, forbidden-phrase list, scenario table, output format, and all three worked examples are untouched -- only its trailing input-data block, as described above. The 5 post-summary judges' scoring logic, weights, decision thresholds, and worked examples are untouched -- only their input-wiring block. Steps 1-2 (STT/Validation) were already rewritten by the interrupted prior session and were verified, not re-written, against the spec's exact formulas.

---

## Addendum (2026-07-08, same day) — pipeline simplification pass: hybrid Code+LLM check stages, Presentation Judge moved to code, Quality Orchestrator weights/UNAVAILABLE handling

**Task.** A same-day follow-up spec asked to simplify the stages-1-9 rework above without removing any check: Fact/Need/Outcome Check become single stages that internally run a cheap deterministic Code check (schema/dictionary/range validation) *and* an LLM Judge (semantic/evidence/hallucination checks), merged into one criteria list -- rather than the Code check being a separate pipeline position. Presentation Judge, being 100% mechanically checkable (length, PII, address, price, a required status/next-step line), moves from an LLM call to pure code. The Quality Orchestrator's weights change to Truth 40/Critical Facts 20/Context 20/Action 15/Presentation 5 (was 40/20/23/15/2), its decision rule changes (`AUTO_SAVE` score≥90 & no critical errors; `RETRY` critical errors or score 75-89; `MANUAL_REVIEW` score<75 or a conflicting verdict), and a judge that fails from a genuine network/timeout error must be marked `UNAVAILABLE` -- not scored 0 -- excluded from the weighted average (with the remaining judges' weights renormalized) and must not by itself block `AUTO_SAVE`/CRM save. The Need Agent's `property_requirements` also gains real structure (`districts`/`residential_complexes`/`rooms`/`repair`/`floor_preferences`/`hard_constraints`/`rejected_options` arrays + `evidence` + `confidence`) instead of one free-text field, and Conversation Store must keep whichever of those sub-fields verify even if others don't (no longer all-or-nothing). A live bug was also reported and fixed: the client-facing "Итоговое саммари" panel showed literal `[object Object]` for Интересует/Источник средств/Срок покупки, because it read raw `ctx.needs` (now `{value,confidence,evidence,...}` objects per the stages-1-9 rework above) through a formatter that only understood flat strings/arrays.

**Design decisions made explicit** (spec didn't fully specify the merge mechanics):
1. **Hybrid stage type.** Added `type:'hybrid'` (`runStage()`) as a third branch alongside plain `code` and `llm`/`check`: it runs the stage's `codeFn` first (a lightweight validator, *not* a full stage output -- returns `{criteria, fieldStatus, hardFail, issues}`), then calls the LLM with the stage's (now semantically-focused, schema-talk trimmed out) prompt, then merges both via `mergeHybridCheck()` (Fact Check, Outcome Check) or the property_requirements-aware `mergeNeedCheck()` (Need Check). `fieldStatus` acts as a second gate: even if the LLM Judge says a field is `verified:true`, a field the code check marked `fail` (bad schema/dictionary) is force-rejected in the merged output -- both levels must agree.
2. **Score/decision blend.** `score = codeScore*0.3 + llmScore*0.7` (LLM is the semantic authority; code is baseline hygiene), floored to ≤40 and decision forced to `FAIL` if the code check hit a `hardFail` (missing required fields/broken JSON structure) regardless of what the LLM said. Otherwise decision from score: ≥90 `PASS`, ≥75 `PASS_WITH_WARNINGS`, ≥60 `MANUAL_REVIEW`, else `FAIL`.
3. **"Conflict" for MANUAL_REVIEW**, left undefined by the spec, is narrowly interpreted as Truth Judge's `has_role_confusion===true` -- the one signal genuinely indicating the system itself is unsure who said what, warranting a human rather than a mechanical retry. Decision priority (spec didn't say which wins when several conditions overlap): conflict first, then the RETRY rule, then AUTO_SAVE, then MANUAL_REVIEW as the catch-all.
4. **UNAVAILABLE detection** (`isRetryableError()`) matches timeout/5xx/429/network-failure patterns in the caught error message; only applies to the 4 post-summary LLM judges (`truth_check`/`critical_facts_check`/`context_utility_check`/`action_check`), not the hybrid Fact/Need/Outcome Check stages (the spec's "недоступен по API" language was specifically about section 6, the Quality Orchestrator's judges) or Presentation (no longer an LLM call, so it can't be network-unavailable). `normalizeSummaryJudge()` recognizes `raw.unavailable===true` and returns a distinct `status:'unavailable'` (not folded into `warning`/`fail`); `summaryQualityGate()` renormalizes weights over only the non-unavailable judges and lets `passOrUnavailable(key)` (pass OR unavailable, either is fine) gate `AUTO_SAVE` instead of requiring a hard `pass`.
5. **Removed redundancy** (the stated goal): since Presentation Judge is now code and thus 100% trustworthy about its own length/PII/card-duplicate findings, the old "never trust the judge's self-report" deterministic safety nets inside `summaryQualityGate()` (re-running `detectForbiddenSummaryDetails`/length checks a second time and force-overwriting the judge's own fields) were deleted; the gate now reads `presentation.is_too_long`/`presentation.pii_found`/`presentation.forbidden_card_duplicates` directly from the code judge's own output. The style-persuasion (`detectDialogueStyle`) and invented-next-step (`hasInventedNextStep`) deterministic overlays stay, since Presentation's codeFn doesn't compute those.
6. **Conversation Store shape**, per the spec's literal example, dropped the previous `overall_store_score`/`transcript_validation_score` field names in favor of `quality:{fact_check_score, need_check_score, outcome_check_score, store_score}` (the validation score still feeds the `store_score` formula at 20% weight, it's just no longer its own field in the output, matching the spec's exact JSON example rather than the richer shape from the same-day-earlier addendum).

**Verified live, not just by code inspection.** `npm run lint` clean, `npx vitest run` 236/236, `npm run build` succeeds. Beyond that: (1) ran STT+Validation for real in a live browser preview (deterministic code stages) against a new reconstructed "Ксения" transcript, confirmed the pipeline's 17 stages are in the exact right order with the right `type`/`codeFn` per stage (`fact_check`/`need_check`/`outcome_check` → `hybrid`; `presentation_check` → `code`); (2) since no API key is available in this environment, mocked realistic LLM-Judge JSON responses directly into the live page's `ctx` and called its own `factCheckCode`/`needCheckCode`/`outcomeCheckCode`/`mergeHybridCheck`/`mergeNeedCheck`/`conversationStore`/`crm`/`presentationCheck`/`summaryQualityGate`/`displayNeeds`/`flattenPropertyRequirements` functions directly to exercise every path: Fact Check merged 3 code criteria + 1 LLM criterion into score 97 (`0.3*100+0.7*95`) and `PASS`; Need Check's LLM Judge was made to explicitly reject one `property_requirements` sub-field (`rooms`) while approving the rest -- confirmed the merged output kept `repair`/`floor_preferences`/`hard_constraints`/`rejected_options` and correctly dropped `rooms`, landing in `conversation_store.property_requirements` (top-level, not nested) and then in `crm().card.property_requirements`, and `flattenPropertyRequirements()` rendered it as `"с ремонтом, не первый этаж, рядом со школой"`, matching the spec's example format; `conversation_store.quality.store_score` computed to `0.963`, hand-verified against the stated formula; `displayNeeds()` correctly returned the unwrapped `ctx.crm.card` (no more `[object Object]`); `presentationCheck()` correctly scored a clean 145-character summary containing a `Договорились:` line as `100`/`pass`; the Quality Orchestrator was exercised through all four decision branches -- a normal all-pass case, an `action_check:{unavailable:true}` case (renormalized score `92.9` computed by hand from the remaining 4 judges' weights, decision still `AUTO_SAVE`, with a clear warning logged and CRM not blocked), a `critical_facts_check` hard-fail case (`RETRY`), and a Truth-Judge role-confusion case (`MANUAL_REVIEW`, correctly overriding what would otherwise have been a passing score) -- all four produced the intended decision. Also re-ran the full migration test suite (isolated Node module, same technique as the prior addendum): a saved config from the *previous* deploy (already has `fact_check`/`need_check`/`outcome_check` as `type:'check'` and `presentation_check` as an LLM stage, both at `promptVersion:1`) correctly upgrades every one of them to `type:'hybrid'`/`type:'code'` with the right `codeFn`/`vendor` and `promptVersion:2`; a stage with `userEdited:true` was correctly left untouched.

**What was NOT changed.** Stage 10 (Summary) and its `conversation_store`-only input wiring from the same-day-earlier addendum are untouched. The Fact/Need/Outcome Agent extraction prompts (value/confidence/evidence/verification_status schema) are untouched except for the Need Agent's `property_requirements` restructuring described above. Truth/Critical Facts/Context/Action judges' own scoring logic, worked examples, and decision thresholds are untouched -- only the Orchestrator's weights and the renormalization/UNAVAILABLE wrapper around them changed.

---

## Addendum (2026-07-08, later same day) — Reports/Review's importer never actually parsed a real `pipeline_report.json`; fixed and covered by tests

**Reported symptom.** "History stopped saving after all the changes" -- run history that used to show up with real scores/graphs everywhere stopped. Investigated by first finding that **two independent, unrelated** history mechanisms exist, both pure `localStorage`, no server persistence: (A) Dashboard's automatic history (`ai-product-studio.playground-test-runs.v1`, written by `reportRunToParent()`/`persistDashboardRun()` in `public/pipeline-lab-v3.html` on every run, read by `dashboard-screen.tsx`), and (B) the Reports/Review screens' history (`summary_review_runs_v1`, written only by *manually* uploading a downloaded `pipeline_report.json` through `/review`'s file input, via `normalizePlaygroundRun()` in `src/features/summary-review/importer.ts`).

**Root-caused, not assumed.** Rather than trust the first plausible theory, both mechanisms were exercised live: patched `callModel()` inside a real embedded Pipeline Lab v3 iframe (`playground-screen.tsx`'s actual embedding, product `project_demo_call_analysis`) with realistic canned per-stage JSON, ran the full 17-stage pipeline for real, and confirmed **mechanism A works correctly** -- a run was written to `localStorage` with sane `confidence`/`qualityScore`/`decision` values, ruling out the "recent rework broke the auto-save path" theory. Mechanism B was a different story: `normalizePlaygroundRun()` (written in `e39642e`, 2026-07-07, *before* either same-day rework above) was hand-written against an assumed flat contract (`client_name`, `summary` as a string, `ai_score`, a single `judges` bag) that **never actually matched** what `public/pipeline-lab-v3.html` produces -- `result` (aliased `ctx`) has always nested these under `facts.client_name.value`, `summary.summary` (an object, not a string), `summary_quality_gate.summary_quality_score`/`.decision`, and 5 separate top-level judge keys. Every `pick*` helper silently fell through to its fallback (`""`/`0`/`"MANUAL_REVIEW"`/`{}`) without throwing -- so uploads always "succeeded" while quietly recording nothing useful, exactly matching the reported "scores are wrong/history looks broken" without any visible error. The two same-day pipeline reworks above didn't cause this -- they widened a gap that already existed from day one, since `importer.ts` was never touched by either.

**Fix (`src/features/summary-review/importer.ts`).** Added `pickFieldValue()` (unwraps a `{value,...}` field, falls back to a flat value for pre-rework reports), `pickClientName()` (prefers `conversation_store.facts.client_name.value`, falls back to raw `facts.client_name.value`, falls back to old flat keys), `pickSummaryText()` (unwraps `{summary: "..."}`), `pickJudgesBag()` (collects the 5 separate `*_check` keys into one object, falls back to old single-bag keys), and reads `aiScore`/`aiDecision` from `summary_quality_gate`/`gate` first. Every new helper still falls back to the original flat-key lookups, so an old/legacy flat report still imports the same as before.

**Verified live, not just by code inspection.** Added `src/features/summary-review/importer.test.ts` (7 cases: real nested shape, legacy flat shape, empty-input fallback) -- `npx vitest run` 248/248 (was 236, +12 across both same-day pipeline addenda and this one). Beyond unit tests: reused the same mocked-iframe technique to produce a genuine `pipeline_report.json` from a full run, then drove the *actual* `/review` file-input UI (constructed a real `File`/`DataTransfer`, dispatched a `change` event -- not a direct function call) and confirmed the rendered page showed the real client name ("Дмитрий"), the real transcript, the real Summary Agent text, and a real computed `AI Score`/`AI Decision` (`74.5`/`RETRY`, whatever the actual Quality Orchestrator produced for that mocked run) instead of the previous silent `"Клиент не указан"`/`""`/`0`/`"MANUAL_REVIEW"` defaults. Then navigated to `/reports` and confirmed the same run appears correctly in its table (client name, AI Score, AI Decision all populated) and its "Доля RETRY" aggregate reflects it.

**What was NOT changed.** Mechanism A (Dashboard's automatic history) needed no fix -- confirmed working as-is. `/api/import-playground-run` remains unused dead code (confirmed via repo-wide grep, not called from anywhere) -- out of scope for this fix since nothing feeds it.

---

## Addendum (2026-07-10) — scoring-honesty pass: user-edited checker format now readable by code stages, card-context filter, honest per-stage scores

**Reported symptoms**, from a real run of the "Модуль транскрибации и AI-саммари звонков" product (its pipeline is fully user-edited: custom stage names, custom outKeys `fact_judge`/`need_judge`/`outcome_judge`/`conversation`/`context_check`/`quality_gate`, custom prompts returning `{scores:{overall:0-100}, decision}` instead of the canonical `{*_check_quality:{score:0-1}}`): (1) Валидация 66%/RETRY_STT, Хранилище 13%/FAIL, Summary Quality Gate 35/RETRY with 7 critical errors -- despite the judges themselves scoring 94/91/88 PASS/WARNING/WARNING; (2) every extraction/checker stage displayed an identical "Оценка: 90%".

**Root causes, each confirmed by replaying the actual `pipeline_report (24).json` through the code:**
1. **90% everywhere** -- `stageScoreFromOutput()`'s llm/check fallback returned a hardcoded 90 for any parsed-JSON output lacking a top-level `score`; the custom checkers' real scores live in `scores.overall`, which nothing read.
2. **Хранилище 13%** -- `conversationStore()` read quality only from the canonical `fact_check_quality.score` keys; the custom shape read as three zeros, so `store_score = 0.66*0.2+0+0+0 = 13.2%` → FAIL, even though the data itself (18 facts/11 needs/5 outcome fields) was collected fine.
3. **Gate 35** -- three stacked problems: (a) the user-edited critical-facts judge prompt lacks the "КАРТОЧКА CRM" carve-out, so it demanded address/floor/price/name/phone in the summary -- exactly what the format judge forbids, an unsatisfiable contradiction producing 5 phantom criticals; (b) the existing `splitCriticalFactsBySummaryNeed` card-context filter silently failed because `structuredStore()` read only `ctx.conversation_store` while this pipeline's store stage writes `ctx.conversation` (same alias bug left `crm()`'s card empty -- `client_name: null` etc.); (c) the critical penalty formula (`min(score, max(35, 74-(n-1)*8))`) crushed 87→35 on the back of those phantoms.
4. **Валидация 66%** -- three deterministic false positives: "— Угу."/"— Ну да." counted as damaged STT (`shortNoise`: every word is a filler), a long client turn (≥35 words + domain words) counted as role confusion, and a short closing confirmation ("— Угу, хорошо.") counted as a cut-off ending.

**Fixes (all in `public/pipeline-lab-v3.html`, one mirror line in `playground-dashboard-analytics.ts`):**
- `stageScoreFromOutput()` reads `scores.overall` and a `decision`-based fallback (PASS→92/WARNING→80/MANUAL_REVIEW→65/FAIL→40); an extraction stage with no self-score at all now returns **null**, rendered as "—" ("оценивает следующий проверщик") instead of a fabricated 90% -- `buildStageMeta`/`metaStatusFromScore`/`renderReport` all handle null. Mirrored `scores.overall` into the React Dashboard's `scoreFromRecord()` (null `meta.score` was already safely skipped there).
- `conversationStore()`: new `checkScore01`/`checkReadable`/`checkDecision` helpers accept canonical, `scores.overall` (either scale), and decision-only shapes; `schemaInvalid`/`hasVerifiedIntent`/`needsManualReview` no longer hard-require the canonical shape (intent-required applies only when the canonical fact schema is in use).
- New `conversationStoreValue(ctx)` alias (`conversation_store||conversation`) used by `structuredStore()`, `crm()`, and `displayNeeds()` -- un-breaking the card-context filter and the CRM card for the custom pipeline.
- Gate: `isForbiddenInSummaryFact()` unconditionally demotes missed-fact claims about fields the format judge bans from the summary (address/street/price/floor/area/phone/client name) to `card_context_facts`; the split now runs **before** the weighted score; when a failing critical-facts judge's misses were *all* card-context, its verdict is deterministically corrected to pass/≥90 (per its own scoring rules) with an explanation; the critical penalty is now proportional (`max(30, score - 8*n)`) instead of the 74-cap/35-floor crush; ≥2 retelling-style hits now surface in `key_problems`/`main_reason`.
- Validation: word-by-word backchannel whitelist (≤4 words, all from угу/ага/да/ну/хорошо/... set, leading dash/punctuation stripped) exempts live confirmations from `isDamagedPhrase`; the ≥35-word client-turn role-confusion heuristic is removed (the two precise phrase-based rules remain); `hasCompleteEnding` accepts short closing confirmations -- with the Cyrillic-`\b` pitfall found live and fixed via an explicit lookahead (JS `\b` is ASCII-only and silently never matches after a Cyrillic word).
- `shortIssue()`'s first branch dropped the greedy `|следующ` token that turned "последующие консультации" into a phantom "Убрать выдуманный следующий шаг" recommendation.
- `migratePipelineConfig()` gained a targeted, **additive** hotfix for user-edited critical-facts judges: if the prompt lacks "КАРТОЧКА CRM", a card-context block is prepended once (idempotent contains-check); the user's own text is not altered by a single character -- the first deliberate, narrowly-scoped exception to "never touch userEdited prompts", justified because the missing block makes the judge structurally contradict the format judge. `PIPELINE_VERSION` 11 → 12 (metadata only, per V8.4).

**Verified by replaying the user's actual report** (extracted the changed functions into isolated Node harnesses -- same technique as prior addenda): Валидация 66%/RETRY_STT → **99%/PASS**; Хранилище 13%/FAIL → **92%/PASS** (fact/need/outcome scores now read 0.94/0.91/0.88 as the judges actually scored); Gate 35/RETRY-with-7-criticals → **87/RETRY with exactly 1 genuine critical** ("Summary написано как пересказ диалога" -- a real style violation per V8.7 policy, so RETRY remains the correct decision; pre-penalty score 95, all 5 phantom card-fact criticals demoted to `card_context_facts`); stage scores 90/90/90/90 → **—/94/91/88**. The CARD-block injection was replayed against the report's own saved 16-stage config: block inserted, user text byte-preserved, idempotent on second run, other user-edited prompts untouched, stage order/count unchanged (with the envelope's `deletedStageOutKeys` honored, matching real-browser behavior). `npm run lint` clean, `npx vitest run` 251/251, `npm run build` succeeds; live-browser smoke test confirmed version 12, all new functions present, backchannel/ending/scores.overall/null-score behaviors correct, zero console errors.

**What was NOT changed.** The judges' own prompts (beyond the additive card-context block for the critical-facts judge), the RETRY decision rule (critical errors still force RETRY -- only the *score* stopped being crushed), the canonical default pipeline's stages/prompts, and the deleted-stage persistence mechanism (`deletedStageOutKeys`, from the parallel "Fix sandbox pipeline step persistence" work) are all untouched.

---

## Addendum (2026-07-21) — provider/model reliability pass: judges on gpt-5-mini, full transport-param propagation, no vendor fallback for judges, live summary auto-retry

**Reason.** An external audit of the deployed Playground found five configuration/transport defects that made the "Модуль транскрибации и AI-саммари звонков" pipeline unable to pass its own quality loop reliably: (1) four judge stages (truth_check + the three hybrid checks) pinned model id `claude-sonnet-4-6`, which exists neither in the app's own model dropdown (`claude-sonnet-4.5`, dot notation, the AI Tunnel alias) nor as an Anthropic API id (dash notation, `claude-sonnet-4-5`); (2) `callAnthropic` hardcoded `max_tokens:1500` (judge contracts need 2500-6000), dropped `temperature`, and never surfaced `stop_reason`, so truncation looked like "judge returned invalid JSON"; (3) `/api/openai-proxy` accepted only `{apiKey, model, prompt}` -- `temperature:0`, `maxTokens`, and the strict `response_format` json_schema for Fact Agent never reached OpenAI; (4) `callModel`'s cross-vendor fallback could silently swap a judge to the same model as the generator (self-review); (5) `retrySummaryQualityFlow` triggered only on `gate.decision==='RETRY'`, which Summary Quality Gate v1 never emits (its enum is AUTO_SAVE/SAVE_WITH_WARNING/REVIEW_REQUIRED/TECHNICAL_ERROR) -- dead code, so no bad summary was ever regenerated. The product owner chose gpt-5-mini as the judge model (п1) and asked for the rest to be fixed and deployed.

**Changed in `public/pipeline-lab-v3.html`:**
- **Models.** All four claude-pinned judges → `model:'gpt-5-mini', temperature:0, maxTokens:6000` (they previously had *no* explicit temperature/maxTokens at all, falling to the 0.2/2000 transport defaults); the four `deepseek-v3.2-exp` post-summary judges → `gpt-5-mini` (a judge should not be weaker than the generator it scores); per-stage `promptVersion` bumped on all eight so the existing content-refresh migration pushes the model change to saved-but-never-edited browser configs (a `userEdited` stage keeps its own model, per V8.4). The four legacy `migratePipelineConfig()` forced-assignment blocks, the new-stage defaults (`addStage`, legacy no-model fill-in), `MODEL_VENDOR`/`COST_PER_1K_TOKENS` (added `claude-sonnet-4.5`/`claude-sonnet-4-5` entries), and the "Claude Sonnet 4.6" UI strings were updated to match.
- **Transport.** `callAnthropic(prompt, model, temperature, maxTokens)` now passes real temperature/max_tokens, maps `stop_reason:'max_tokens'` → `finishReason:'length'` (so the existing TRUNCATED_JSON detection finally works outside AI Tunnel), and normalizes model aliases (`ANTHROPIC_MODEL_ALIASES`: `claude-sonnet-4.5`→`claude-sonnet-4-5`, legacy `claude-sonnet-4-6`→`claude-sonnet-4-5`), reporting the resolved id as the stage's actual model. `callOpenAI` forwards `temperature/maxTokens/responseFormat` and returns `finish_reason`. `callAiTunnel` and the proxy share a reasoning-model guard (`^(gpt-5|o\d)`): such models get `max_completion_tokens` and no `temperature` (OpenAI rejects both otherwise). `callModel` threads all params through every branch, including both fallback paths (fallback claude id corrected to `claude-sonnet-4-5`).
- **Judge fallback ban.** `stageAllowsVendorFallback(stage)` (false for `type:'check'|'hybrid'`) threads a new `allowFallback` flag through `callModelWithTransientRetry`/`callModel`: a judge whose vendor call fails now errors honestly instead of being silently re-scored by the other vendor's model. Generator/extraction stages keep the fallback.
- **Summary auto-retry is now live.** `summaryQualityRetryable(gate)` replaces the dead `decision==='RETRY'` check: retry triggers on legacy `RETRY` (user-edited `gate` codeFns still emit it -- contract preserved, covered by the existing smoke tests) *or* on a v1-gate `REVIEW_REQUIRED` whose cause is summary-side (no technical_errors, store READY/READY_WITH_WARNINGS, summary GENERATED, no `conversation_store`-sourced hard stops -- store-side problems are not fixable by regenerating the summary). Each attempt now injects `buildSummaryRetryFeedback(ctx)` (gate hard stops + the judges' own structured complaints, deduped, top-12) into the summary prompt via `moduleSummaryPrompt` -- without feedback, a temperature-0 retry would reproduce the same text and just burn tokens. The retry loop also clears the CRM run-scoped idempotency cache and stamps a fresh `__crm_stage_execution_id` per attempt, so the improved result actually saves instead of replaying the first attempt's queued-review record.
- **UI honesty fix.** `stageScoreFromOutput()` returns `null` for `NOT_RUN` outputs -- cascade-blocked stages no longer display a fabricated "Оценка: 100%".

**Changed elsewhere:** `src/app/api/openai-proxy/route.ts` (forwards `temperature`/`maxTokens`/`responseFormat` with the same reasoning-model translation; backward-compatible with the old 3-field body); `src/shared/llm/browser-direct-provider.ts` (same alias map, `DEFAULT_ANTHROPIC_MODEL` → `claude-sonnet-4-5`, `MODEL_VENDOR` kept in exact sync with the HTML per `model-catalog-sync.test.ts`); `src/features/mvp/screens/settings-screen.tsx` (label text); `tests/smoke/pipeline-summary-audit.spec.ts` (4 default-config assertions updated to the new models/promptVersion; tests that build their own deepseek-shaped mock stages were deliberately left alone).

**Verified.** `npm run lint` clean; `npx vitest run` 281/281 (the model-catalog sync test caught the HTML/TS drift mid-change, then passed); `npx playwright test --project=chromium` 88/88 (three initially failed: one stale deepseek assertion, two encoding the legacy RETRY contract -- the latter drove the "legacy RETRY stays retryable" decision above rather than a test edit); `npm run build` succeeds; `node --check` on the extracted inline script; live in-browser run (fresh localStorage, mock provider) confirmed all 12 model stages load as gpt-5-mini with explicit temperature/maxTokens, the 17-stage run completes with the correct facts-failure cascade, NOT_RUN cards show "не рассчитана" instead of 100%, and zero console errors. Real-model behavior (AI Tunnel/OpenAI/Anthropic keys) can only be confirmed against live keys post-deploy.

**What was NOT changed.** Stage prompts' text, the Quality Gate's criteria/weights/thresholds (`auto_save_min_total:95` etc. -- calibration against a golden dataset remains open), the STT/CRM simulations, and the localStorage-only persistence model are all untouched.

**Post-deploy follow-up (same day): the promptVersion refresh was unreachable for contract-current saved configs.** Verified live against the deployed production site with a previously-saved config: the migration did NOT upgrade the judge models, because the second `migratePipelineConfig()` forEach began with `if(contractKeys.has(canonicalKey)&&stage.contractVersion===TRANSCRIPTION_SUMMARY_CONTRACT_VERSION) return;` -- an unconditional skip for every contract stage already stamped with the current contract version, which is *every* stage of every config saved since the 2026-07-08 rework. That made the per-stage promptVersion content-refresh (the 2026-07-07 fix's whole point) dead code for exactly the browsers that needed it -- the same "structural check blocks content refresh" bug class as the 2026-07-07 addendum, reintroduced one layer up. Fix: the contract-current check now guards only the structural blocks (summary provider force-set, check→hybrid type conversion); the promptVersion refresh runs unconditionally after it, and now also syncs `temperature`/`maxTokens` from the default when defined (the judges' new explicit limits would otherwise never reach saved configs). Verified live in-browser both ways: a saved config downgraded to the old models/promptVersions auto-upgraded all 8 judges to gpt-5-mini with correct temperature/maxTokens on reload, and a `userEdited:true` stage with a custom prompt/model survived the same reload byte-identical.

---

## Addendum (2026-07-21, later same day) — Quality Gate calibration, real Nexara STT, Postgres-backed durable storage

**Purpose.** Three follow-up deliverables after the model/transport fixes above: (1) the Summary Quality Gate's AUTO_SAVE threshold was, by design, almost unreachable on real judge output -- calibrated against a new golden dataset; (2) real Nexara audio transcription, replacing the always-simulated `transcript_quality_check` path when the user has no audio to upload; (3) a Postgres-backed durable store layered additively under the existing localStorage repository, so real data survives a cleared browser/new device/redeploy.

**1. Quality Gate calibration (`public/pipeline-lab-v3.html`, `moduleSummaryQualityGateV1`).** The old AUTO_SAVE rule required literally every one of the 5 post-summary judges to report zero warnings simultaneously (`statuses.every(status=>status==='pass')&&!warnings.length`, plus `auto_save_min_total:95`/`auto_save_min_criterion:90`) -- on real independent LLM judges this combination was reached almost never, since any one of five judges finding even a trivial nit collapsed every run to `SAVE_WITH_WARNING`, regardless of how good the summary actually was. Root-caused by discovering `summaryQualityGateCheckerConsistency()`'s own invariant: `status:'pass'` is only valid at `score>=95` -- meaning the old rule's real effective per-criterion floor was already 95, not the documented-but-dead 90.

Recalibrated: Truth (highest-weighted, the only criterion with direct business risk -- hallucination/PII/money errors) keeps zero tolerance for warnings, unchanged. The other four judges (Critical Facts/Utility/Action/Presentation) may now carry **at most 1 total warning** between them; `auto_save_min_total` relaxed 95→90, `auto_save_min_criterion` relaxed 90→85 (both now real thresholds in `SUMMARY_QUALITY_GATE_THRESHOLDS`, actually read by the decision logic instead of the previous hardcoded 95/90/90/80 magic numbers the config object never fed into). Every other hard requirement (Store READY, Summary GENERATED, next_step_verified, can_continue_without_recording, character_limit_valid, zero missing_items, all REVIEW_REQUIRED hard-stop triggers) is unchanged.

New file `tests/smoke/quality-gate-golden-dataset.spec.ts` -- 9 hand-labeled cases (CLAUDE.md §26's first real golden dataset in this repository) spanning clean/one-minor-nit/two-nits/exact-boundary/truth-warning/hallucination/store-manual-review/low-action-score, each asserting the calibrated decision with a one-line rationale. Two pre-existing tests in `pipeline-summary-audit.spec.ts` (a "single presentation warning, otherwise clean" gate case and its CRM save-mode consequence) changed expected decision from `SAVE_WITH_WARNING`→`AUTO_SAVE`, each with an inline comment explaining why -- this is the deliberate calibration effect, not an accidental regression; every other pre-existing gate-boundary test (all-warning, one-truth-warning, per-criterion-fail-below-80, hard-stop cases) was checked and confirmed unaffected.

**2. Real Nexara STT.** Previously `evaluateSttContract()`'s `hasAudio` branch (`real_stt` mode) was unreachable -- the UI only ever offered a text transcript textarea, so every run silently used the simulated `transcript_quality_check` path regardless of the `provider:'Nexara'` label in stage output. Endpoint/auth/request-shape sourced from Nexara's public docs (`docs.nexara.ru`, fetched 2026-07-21 -- not exercised against a live account in this environment, no key available here): `POST https://api.nexara.ru/api/v1/audio/transcriptions`, `Authorization: Bearer <key>`, multipart with `file`/`response_format`/`task`/`num_speakers`/`language`.

New `src/app/api/nexara-proxy/route.ts` (same stateless-relay pattern as `openai-proxy`, multipart passthrough, key never logged/stored). New `pipelineLabV3.nexaraApiKey` BYOK key (Настройки screen + Pipeline Lab v3's hidden API-ключи card, same convention as Anthropic/OpenAI/AI Tunnel). New audio-upload control in the "Вход" card (`public/pipeline-lab-v3.html`): uploading + "Расшифровать" calls Nexara for real, parses `verbose_json` `segments[]` for per-speaker diarization (`speaker`/`speaker_id`/`speaker_label`, tried in that order -- best-effort, since the exact response field name wasn't independently confirmed), and populates the transcript textarea with `Спикер N:`-labeled lines. Deliberately **not** auto-mapped to Клиент/Агент/Оператор and **not** auto-run: Nexara diarizes speaker *identity*, not semantic *role*, and guessing that mapping would be exactly the kind of fabricated certainty AD-1/AD-6 forbid -- the UI prompts the user to verify/relabel roles before running the pipeline (a real Human Review trigger per §13's own "policy uncertainty" criterion). `runPipeline()` only attaches the real `stt_metadata` to `ctx` when the transcript textarea still matches what Nexara returned (edited-away text falls back to the simulated path, unchanged from before). Zero behavior change when no audio is uploaded or no key is configured -- confirmed live end-to-end with a mocked proxy response (3-segment 2-speaker fixture correctly diarized and populated).

**3. Postgres-backed durable storage, additive.** `ProjectRepository.load()/save()` (`src/shared/repositories/types.ts`) are synchronous by design (§8.7's own doc comment: "cheap for a client-held, Local-Storage-backed repository") and are called by 19 files across the app, including on every single mutation via `setSnapshot`. Converting this interface to async to support a real network-backed store outright is the migration §63 debt item 5 already deliberately deferred -- doing it as a rushed side effect of this task would be exactly the kind of large, high-risk architectural change §41/§69 say needs its own scoped initiative, not a same-session bolt-on. Implemented instead as a **best-effort background layer** that changes zero synchronous behavior for any existing caller:

- New `src/shared/repositories/postgres-store.ts` (server-only: reads `POSTGRES_URL`/`DATABASE_URL`/`POSTGRES_URL_NON_POOLING`, whichever is set -- resolved manually rather than relying on `@vercel/postgres`'s own default, which only checks `POSTGRES_URL`, since the exact env var name the Neon marketplace integration sets was not independently confirmed). One `repository_snapshot(id, data jsonb, updated_at)` table -- the whole `RepositorySnapshot` as one JSON document, not 10 relational tables, because `ProjectRepository`'s per-entity `upsert*` methods are (per §8.7) called by zero production code; a relational schema for entities nothing queries individually would be complexity with no present benefit (§2 principle 6).
- New `src/app/api/repository/route.ts` (GET/POST, `RepositorySnapshotSchema` validation reused from `local-storage-repository.ts`, now exported for this reason). Returns `{configured:false}` (200, not an error) when no connection string is set, so the client-side sync no-ops cleanly rather than treating "no DB provisioned yet" as a failure.
- `repository-store.ts`'s `load()` captures whether this browser already had local data *before* calling `projectRepository.load()` (which auto-seeds demo data on a first run, erasing that signal) -- a genuinely fresh browser adopts the server's snapshot once the background fetch resolves; a returning browser's local data is never silently overwritten. `setSnapshot()`/`reset()` fire-and-forget a POST to the server after their existing synchronous local save, unchanged otherwise. A network failure or missing DB is logged to console and swallowed, never thrown.

**Explicit scope limit, stated per AD-6 (no overclaiming):** this is a single shared workspace, not a multi-user backend -- there is still no auth in this application (§10 SB-1), so every visitor to the deployed URL reads/writes the same row. It solves "data survives a cleared browser / redeploy," not "each user has their own data." Real multi-tenancy needs auth first, a distinct, larger initiative.

**Provisioning status.** `@vercel/postgres` installed. `npx vercel integration add neon --claim --no-env-pull` was run twice from this environment and both times returned `action_required: integration_terms_acceptance_required` -- Vercel's marketplace requires a human to accept Neon's terms in an authenticated browser session at the tool-provided `verification_uri`; this cannot be done by an agent and was not attempted. **The database is not yet provisioned** -- until the product owner opens that link, accepts terms, and either re-runs the install command or provisions Postgres from the Vercel dashboard (Storage tab) and sets `DATABASE_URL`/`POSTGRES_URL`, `/api/repository` reports `{configured:false}` and the app behaves exactly as before this change (localStorage-only). All code is deployed and ready to activate the moment the connection string exists -- no further deploy needed once the env var is set (Vercel Route Handlers read `process.env` at request time).

**Verified.** `npm run lint` clean; `npx vitest run` 283/283 (2 new tests for `/api/repository`'s not-configured fast path); `npx playwright test --project=chromium` 97/97 (9 new golden-dataset cases, 2 recalibrated assertions, zero regressions elsewhere -- including the full existing Project-screen/dashboard suite, confirming the background-sync addition doesn't disturb any existing repository-store consumer); `npm run build` succeeds (`/api/repository` and `/api/nexara-proxy` both present as dynamic routes); `tsc --noEmit` clean; live in-browser: Nexara key field renders and persists in Настройки, audio upload enables "Расшифровать", a mocked Nexara response correctly diarizes and populates the transcript with a role-verification prompt, and `/api/repository` returns `{configured:false}` cleanly with zero console errors/warnings on every existing screen with no database configured.

**What was NOT changed.** `ProjectRepository`'s synchronous interface itself (unconverted, per the scope discussion above), the CRM simulation, every other Quality Gate hard-stop trigger, and the demo seed data are all untouched.

---

## Addendum (2026-07-22) — real-run outage: Outcome Check hard-blocked the entire pipeline over a dropped `id` and a Cyrillic/Latin channel mismatch

**Reported symptom.** A real production run of "Модуль транскрибации и AI-саммари звонков" completed stages 1-6 successfully (STT through Outcome Agent), then stage 7 "Проверка результата звонка" failed with `JSON не распознан: verified_call_results[0]: matching input result not found`, and stages 8-15 (Conversation Store, Summary, all 5 post-summary judges, Quality Gate) all reported `NOT_RUN` -- a single stage 7 failure silently killed 9 downstream stages.

**Root cause 1 -- id-matching had no fallback.** Stage 6's Outcome Agent produced one `call_results` item (`id:"result_1"`). Stage 7's real gpt-5-mini judge verified it correctly in substance (same `value`) but its JSON response omitted the `id` field and rewrote `evidence` with added speaker labels/quote marks. `normalizeOutcomeJudgeOutput()`'s `resultFor()` matched by `id` when present, else by exact `value`+`evidence` string equality -- with `id` missing and `evidence` reworded, neither path matched, so it threw `OutcomeJudgeSchemaError('matching input result not found')`. This error class (`parseErrKind==='schema'`) was, by explicit prior design, excluded from the hybrid stage's retry logic for exactly the three stages this could happen on (fact_check/need_check/outcome_check) -- so the stage hard-failed on the first and only attempt.

**Fix 1a (matching resilience).** `resultFor()` (call_results) and the new `agreementFor()` helper (verified_agreements/rejected_agreements, replacing direct `agreementsById.get(item?.id)` calls) now fall back, when `id` is absent or unrecognized, to the single remaining unclaimed candidate (unambiguous when there's only one, which is the common case) and then to a `value`-only match. The per-field "Judge changed input value" integrity guard (still enforced for `id`/`value`/`confidence`/`verification_status`, protecting against the judge fabricating a different verdict) no longer requires `evidence` to be byte-identical -- it's supporting citation text the judge may legitimately reformat, not the verified claim itself.

**Fix 1b (bounded retry).** The hybrid stage's retry gate (`runStage()`) now retries once on *any* parse error for fact_check/need_check/outcome_check, including schema-class errors -- previously only plain JSON-parse failures retried on these three stages. The retry prompt quotes the actual validation error back to the model with an explicit instruction to copy `id` verbatim from the input. Same hard failure as before if the retry doesn't help (e.g. a genuinely fabricated/unknown id); a saved run if it does -- strictly non-regressive. `tests/smoke/pipeline-summary-audit.spec.ts`'s `unknownCalls` assertion (a mock that always returns the same broken id, used to assert "no retry happens") was updated from `toBe(1)` to `toBe(2)`, with the final `technical_error` outcome unchanged, since the mock's broken response can't be fixed by any number of retries.

**Root cause 2, found while writing the regression test for root cause 1.** Building a fixture that faithfully reproduced the real transcript ("Скину видео. — Ну, давайте, я жду от вас видео. — Да, давайте в Макс лучше.", `channel:"MAX"`) still failed the merge with `invented_channel` -- a completely separate bug: `outcomeSemanticGuards()`'s channel-grounding check does `evidence.includes(normalized(item.channel))`, a plain string `includes()` between `"max"` (the Outcome Agent's canonical Latin channel value) and `"макс"` (how a Russian client actually says the messenger's name, transcribed in Cyrillic) -- two different Unicode scripts that can never substring-match, so this real client's genuinely-grounded agreement was deterministically flagged as invented on every single run, independent of and unmasked only once fix 1 let the pipeline reach this check at all.

**Fix 2.** New `CHANNEL_TEXT_ALIASES`/`normalizeChannelAliases()` maps common Cyrillic transliterations (макс→max, ватсап/вотсап/вацап→whatsapp, телеграм/телега/телегу→telegram) before the channel-grounding comparison, applied in both `outcomeSemanticGuards()` (agreement-level) and the `primary_next_step` channel check. **First implementation used `\b` word boundaries and silently failed** -- confirmed live: JavaScript's `\b` is defined only over ASCII `\w` (`[A-Za-z0-9_]`), so it does not reliably bound Cyrillic words; `"давайте в макс лучше".replace(/\bмакс\b/g,'max')` left the string completely unchanged in a live test. This is the exact same bug class already documented and fixed once before in this file for `hasCompleteEnding()` (2026-07-10 addendum) -- reintroduced here by not checking for it, now fixed with explicit negative lookahead/lookbehind (`(?<![а-яa-z0-9])макс(?![а-яa-z0-9])`) instead of `\b`, and confirmed live that the corrected pattern actually replaces the match.

**New regression test**, `tests/smoke/pipeline-summary-audit.spec.ts` ("...восстанавливается после пропущенного id и переформулированного evidence..."): reproduces the real production `call_results`/`agreements`/judge-verdict shapes verbatim and asserts the stage now resolves to `PASS` with the original `id`s correctly restored, rather than a technical error.

**Verified.** `npm run lint` clean; `npx vitest run` 283/283; `npx playwright test --project=chromium` 98/98 (97 prior + 1 new regression test; the one `unknownCalls` assertion updated per fix 1b above, confirmed its final outcome is unchanged); `npm run build` succeeds; `node --check` on the extracted inline script; the regex fix was independently confirmed live in-browser (old `\b` pattern: no replacement; corrected lookaround pattern: correct replacement) before being applied to the shipped code, not assumed from reading the pattern alone.
...
## Addendum (2026-07-22, third same-day fix) — real-run outage: stale codeFn silently misrouted need_check/outcome_check to a contract-incompatible merge function, and Need Agent dropped verified budget/area into no requirements at all

**Reported symptom.** After manually fixing steps 3/5/7's model back to gpt-5-mini/AI Tunnel, a real run still failed: stage 4 (Need Agent) returned an empty needs object despite the transcript having a clear budget and minimum-area constraint; stage 5 (Проверка потребностей) reported `decision:"FAIL"` even though every one of its own criteria said "pass"/"warn", with `verified_needs:{}`; stage 7 (Проверка результата звонка) reported `error_code:"PASS_WITH_EMPTY_VERIFIED_OUTCOME"` even though all 5 of its own criteria said "pass" and the underlying Outcome Agent data was complete and correct; Conversation Store then failed with `DEPENDENCY_ERROR`, cascading NOT_RUN through everything after it.

**Root cause 1 -- silent merge-function misrouting via stale `codeFn`.** `runStage()`'s hybrid branch decides which merge function to use via `isNeedCheckStage`/`isOutcomeCheckStage` (`stage.codeFn==='needCheckCode'|'outcomeCheckCode' && /^need_(?:check|judge)$|^outcome_(?:check|judge)$/.test(stage.outKey)`). When these are true, the stage routes to the dedicated `mergeNeedCheckV2()`/`mergeOutcomeCheck()`, which read the *actual* Need/Outcome Judge prompt's real field names (`verified_attributes`/`verified_requirements`, `verified_call_results`/`verified_agreements`/`verified_primary_next_step`). When `codeFn` doesn't match -- which can happen for a browser's saved config once `stage.contractVersion` is already stamped, since `migratePipelineConfig()`'s structural-migration block previously only force-corrected `type`/`codeFn` for *un-initialized* contract stages -- routing silently falls through to the generic `mergeHybridCheck(codeResult, parsed, parseErr, {verifiedKey:'verified_needs'|'verified_outcome', ...})`. That generic function reads `llmParsed[verifiedKey]`, a field name (`verified_needs`/`verified_outcome`) that **does not exist anywhere in the real Need/Outcome Judge prompt contract** -- so `llmVerified` is always `{}` regardless of what the judge actually verified, correctly-passing individual criteria and all. `mergeHybridCheck`'s own `PASS_WITH_EMPTY_VERIFIED_OUTCOME` consistency guard then (correctly) catches the resulting empty-verified-despite-PASS contradiction and hard-fails the stage -- the guard did its job; the bug was upstream, in the merge function it was given.

**Fix 1.** `migratePipelineConfig()` now unconditionally re-asserts `stage.type='hybrid'` and `stage.codeFn=def.codeFn` for `fact_check`/`need_check`/`outcome_check` on every load, *before* the `contractCurrent` early-exit -- these are the module's own structural contract fields, not a user customization, so they're never left to drift regardless of `contractVersion`/`userEdited` state. Provider/model/temperature are untouched by this block, so a user's own vendor/model choice (e.g. switching to AI Tunnel/gpt-5-mini) survives exactly as before. New test in `pipeline-summary-audit.spec.ts` reproduces a stage stuck with `type:'check', codeFn:'gate'` under an already-current `contractVersion` (exactly the drifted shape this fix targets) and asserts migration restores `type:'hybrid'` and the correct `codeFn` for all three stages while preserving `provider:'ai-tunnel', model:'gpt-5-mini'`.

**Root cause 2 -- Need Agent extraction silently drops verified budget/area into empty requirements.** Separately, gpt-5-mini's own Need Agent call returned `requirements:[]` despite Fact Check having already verified `budget_max` ("до пяти с половиной" → 5,500,000 ₽) and `minimum_area` ("минимум шесть соток" → 6) -- a real extraction miss, not a code bug, but one the deterministic fallback in `normalizeNeedExtractionSemantics()` is specifically meant to catch and doesn't always: its `price_limit` derivation only matched a fact's own free-text `evidence`, not its already-verified `category`/`name` classification, so a colloquial evidence phrasing ("у меня просто цена до пяти с половиной") without the literal trigger words could fall through; there was no deterministic fallback for `area` requirements at all.

**Fix 2.** The `price_limit` derivation now also matches on the fact's own verified `category`/`name` (`budget_max`/`client budget`/`price limit`, excluding `object`/`listing`/`объект`/`объявлен`-tagged facts) as an alternative to the evidence-text regex -- a fact Fact Check already classified as the client's budget is trusted as a requirement candidate regardless of how casually it was phrased. A new parallel `area` derivation catches `minimum_area`-category facts or an explicit "минимум N соток/м²" pattern in evidence. New test reproduces the report's exact two facts (`budget_max`→5,500,000, `minimum_area`→6) and asserts both derive into `price_limit`/`area` requirements.

**What was NOT independently re-verified against a live run in this pass** (per AD-6, stated plainly): this fix was reviewed, tested against the automated suite (which includes tests built directly from this real production transcript's facts), and confirmed syntactically/structurally sound, but a full live end-to-end run against real AI Tunnel/gpt-5-mini calls was not re-executed before deploy in this session -- the existing regression coverage (99 Playwright + 283 Vitest, including the two new tests above) is the verification basis.

**Verified.** `npm run lint` clean; `npx vitest run` 283/283; `npx playwright test --project=chromium` 99/99 (2 new tests, zero regressions); `npm run build` succeeds; `node --check` on the extracted inline script.

**What was NOT changed.** The dedicated `mergeNeedCheckV2()`/`mergeOutcomeCheck()` functions themselves, the generic `mergeHybridCheck()`'s own `verified_outcome`/`verified_needs`-empty consistency guard (it was correct to fire -- the bug was in what reached it), and every other stage's routing are untouched.
